...
## Content Portability Test Data (VCST-4872)

### Export/Import Round-Trip Files

Test files for CMS-111 through CMS-122 (save/load/clone functionality, PR #116):

| File | Purpose | Notes |
|------|---------|-------|
| `{page-name}-content.json` | Exported page content | Generated by "Save content" button; valid JSON with blocks/layout |
| `empty.json` | Empty file (0 bytes) | For CMS-121 negative test |
| `malformed.json` | Invalid JSON syntax | For CMS-121 negative test (e.g., `{broken:}`) |

**Note:** Export files are generated dynamically during test execution (CMS-111 Save content). No pre-existing test files required — the round-trip test (CMS-122) exports from PAGE-1 then imports.

### Clone Naming Convention

| Source Name | Clone Name | Clone Permalink |
|-------------|------------|-----------------|
| `QA Homepage Spring Sale` | `QA Homepage Spring Sale (copy)` | `qa-homepage-spring-sale-copy` |
| `QA Homepage Spring Sale (copy)` | `QA Homepage Spring Sale (copy) (copy)` | `qa-homepage-spring-sale-copy-copy` |
| `QA Wholesale Buyer Guide 2026` | `QA Wholesale Buyer Guide 2026 (copy)` | `qa-wholesale-buyer-guide-copy` |

### Content Portability CMS Test Case Coverage

| Test Case | PAGE-1 | PAGE-2 | Description |
|-----------|:------:|:------:|-------------|
| CMS-111 Save content | x | x | Download content JSON |
| CMS-112 Save disabled (new) | — | — | New page, no content |
| CMS-113 Load content | x | — | Upload JSON to create page |
| CMS-114 Load button visibility | — | — | Draft-only toolbar check |
| CMS-115 Load cancel | — | — | No side effects on cancel |
| CMS-116 Clone page | x | — | Clone with content |
| CMS-117 Clone metadata | — | x | Preserves personalization |
| CMS-118 Clone disabled (new) | — | — | New page check |
| CMS-119 Clone of clone | x | — | Double-clone pattern |
| CMS-120 API copy endpoint | — | — | Backend API test |
| CMS-121 Invalid file | — | — | Negative/edge cases |
| CMS-122 Round-trip | x | — | Export → Import fidelity |
