...
--- AMENDMENT — run 2026-09-04 (build delta, one commit) ---

Amended, not re-derived (`modes.md` §Artifact refresh): a fix changes which hypotheses are live, not what
the feature can be wrong about. Same file, same date in the name — a second dated file is the VCST-5346
fork this rule exists to prevent.

Delta under test: **`vc-frontend` PR #2444 commit `a247b3ab` "fix: filter ui issues"**, deployed to this
env 2026-09-04T06:49:07Z (theme `2.57.0-pr-2444-a247-a247b3ab`; "Cloud theme deployment" success).
Backend probed UNCHANGED — SalesRep `3.1007.0-pr-14-5569`, Xapi `3.1021.0-pr-84-0180`, XOrder `3.1010.0`,
platform `3.1063.0` (login-page HTML; `/api/platform/modules` needs an admin token). 17 files, all under
`client-app/modules/sales-rep/`, so the delta is **storefront-only** while the TICKET stays `cross-layer`.

**Part 0 amendment — the delta closes a REVERSE EDGE that Part 0 recorded as absent.**
L3 is *"I narrow it → the list and its option counts follow what I chose."* Its reverse edge — *can the rep
SEE what is narrowing, and UNDO it?* — was carried by the facet-derived checkbox and therefore vanished
exactly when the facets came back empty. That is VCST-5868 in one sentence, and it is a **missing reverse
edge, not a rendering bug**. The commit adds an active-filter **chip row** sourced from `filters` (applied
state) rather than from `statusOptions` (facet-derived state), which is the correct place for it to live.

```mermaid
flowchart LR
  F["applied filters (page state)"] --> C["chip row: one closable chip per active term + Reset filters"]
  F --> Q["salesRepCustomerOrders(filter, facet)"]
  Q --> O["statusOptions / customerOptions (FACET-derived)"]
  O --> D["drawer checkboxes: v-if=statuses.length, so EMPTY on zero-match"]
  C -->|"x removes one term"| A["applyFilters(next), page=1"]
  A --> F
  C -->|":applied prop + deep watch"| D
  O -.->|"chip LABEL is looked up HERE: empty on zero-match, falls back to the raw term"| C
```

New variant: **V7 — active-filter chip row** (present iff >=1 filter applied). It is a variant, not a link:
it is a second UI path onto the existing L3 edge, so it takes matrix rows against L3 rather than a column.

Condition space (delta only): factors = chip kind {status, customerName, startDate, endDate} x facet state
{populated, EMPTY (zero-match)} x culture {en-US, de-DE} x removal path {chip x, Reset chip, drawer Apply}.
4x2x2x3 = 48. **Reduction 48 -> 6**: culture collapses to de-DE alone for the label rows (en-US cannot
discriminate — see #26) and drops elsewhere; `customerName` collapses into `status` for label rows because
its facet has `term === label` by nature (probed: `organizationName` terms are raw org names), which
**refutes** the pre-authoring hypothesis that customer chips mislabel; `drawer Apply` is already covered by
rows #10/#13. Dropped: mobile viewport (rides #25), the two non-date chip kinds' removal (same code path).

| # | Cell | Defect hypothesis | Archetype | Technique | Oracle | Sev |
|---|---|---|---|---|---|---|
| 26 | status chip label, facets EMPTY, culture de-DE | The chip label is `statusOptions.find(o => o.name === status)?.label ?? status`, and `statusOptions` is facet-derived — **empty in exactly the zero-match state the chip exists to serve** — so the label falls back to the raw machine term. The rep sees `Processing` where every other surface says `In Bearbeitung`, on the one screen whose job is telling them what went wrong | FALLBACK | EG | {OBSERVED} API probe 2026-09-04: `term="Processing"` vs `label="In Bearbeitung"` (de-DE) / `"В обработке"` (ru-RU); en-US has term===label and **cannot** discriminate | High |
| 27 | zero-match: is the active filter DISCLOSED and UNDOABLE | VCST-5868's own claim. Three separable failures: the drawer still hides the status group (`v-if="statuses.length"` unchanged by this commit); the chip is absent too; or the chip renders but its x does not clear the term. Disclosure and undo can regress independently | SILENT | ST | {SPEC} `customer-orders.vue` chip row is sourced from `filters`, not `statusOptions` — so disclosure SHOULD now survive an empty facet set | High |
| 28 | close ONLY the `startDate` chip, custom range applied | The drawer resets its preset selector only when **both** dates are empty (`if (!value.startDate && !value.endDate)`), so a half-cleared range leaves the preset control showing a range that is no longer applied — the drawer and the chips disagree about the same state | STALE | DT | {SPEC} the `watch(() => props.applied)` body | Medium |
| 29 | one chip x click, request count | The page emits `change`, sets `filters`, the prop changes, and the drawer's `deep` watch writes `applied`/`draft` back. If that write re-emits, every removal costs two `salesRepCustomerOrders` round-trips and the grid double-renders | RACE | ST | {HYPOTHESIS} -> {OBSERVED} at 3x — one request is correct | Low |
| 30 | drawer option label, long org name (`AGENT-TEST-Org-TechFlow-20260310`) | The label span gained `truncate` and the count moved into a sibling `VcBadge`; a long value is now clipped with no recovery path, and two orgs sharing a 20-char prefix become indistinguishable in the only place you pick between them | RENDER | EG | {BL} BL-UI-002 (overflow) · {OBSERVED} 3 org facet terms on this env are 32-34 chars | Medium |
| 31 | accessible name of the N chip x buttons | `VcChip` names its close button `closeButtonAriaLabel ?? t("ui_kit.accessibility.close_chip")` = **"Close chip"**, and `closeButtonAriaLabel` is passed at **zero** call sites in the whole of `client-app`. With 4 chips a screen-reader user hears four identically-named buttons and cannot tell which filter each one drops | RENDER | EG | {OBSERVED} source `vc-chip.vue` L157 + repo-wide grep: 0 callers · {BL} BL-A11Y-003 | Medium |

Sweeps (delta): `FALLBACK` #26 · `SILENT` #27 · `STALE` #28 · `RACE` #29 · `RENDER` #30,#31. `SCOPE`
**WAIVED** — the delta touches no authorization path (backend unchanged, and the chip row reads page state
the rep already holds). `MONEY`/`BOUNDARY`/`PARITY`/`LIFECYCLE`/`REPLAY`/`CONFIG` **WAIVED** — no money,
no new boundary, no second implementation to diverge from, no lifecycle, no mutation, no new flag.
UIP (delta): UIP-INPUT #26 (culture) · UIP-VIEW #30 · UIP-NET #29. UIP-BACK/DEEP/REFRESH/TABS/EXPIRE/
STORAGE/DATA **WAIVED** — all already carried by #6, #7, #20, #21, #23 and unchanged by this commit.

Variants x links coverage matrix (delta rows only; V7 against the five chain links):

| Variant | L1 open | L2 scope | L3 narrow | L4 breadcrumb | L5 open order |
|---|---|---|---|---|---|
| V7 chip row, facets populated | n/a | n/a | #28, #29, #30 | WAIVED — chips do not touch the breadcrumb | n/a |
| V7 chip row, facets EMPTY | n/a | n/a | #26, #27, #31 | WAIVED — same | n/a |

`n/a` is used only where V7 does not exist on that link (the chip row renders on the list, not on the
profile or the order page); every cell V7 can reach carries a scenario number or a named waiver.

Retired / re-based by this amendment:
- **#21 (typed buyer-page URL, was `{HYPOTHESIS}` Critical) — ANSWERED by the assignee, 2026-09-03.** Order
  access follows **organization membership**, so a rep who serves the org holds full buyer rights on the
  buyer page by design, inherited from the LEO client project. Not a defect; it is the `BL-SR-002`
  membership half working as specified. The row becomes a **`/qa-review-oracles bl` proposal** to state it
  explicitly, and the standing design question ("*should* a rep act as a buyer on a colleague's customer
  order?") is a product decision, not a test outcome.
- **The facet-hiding behaviour itself is declared BY DESIGN** by the assignee the same day ("*adapted from
  the account orders list*"). So #27 asserts **disclosure + undo**, never "the checkbox must stay visible" —
  asserting the latter would file a bug against a stated design decision.
- Existing-coverage triage found **no `REPAIR` and no `RE-BASE`**: `tc:scope --domain sales-rep` over
  `Reset filters` / `status_option` / `Completed (` matched **nothing**, and the corpus never asserted the
  old `"{0} ({1})"` option format. The deleted i18n key has **zero referrers** repo-wide. So this delta
  invalidates no existing row — it exposes an **uncovered surface**, which is what #26-#31 are for.
