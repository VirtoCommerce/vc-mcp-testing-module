...
1. **D2 — RESOLVED + data fixed (2026-07-17).** Root cause: the CSV is correctly labeled (`region_id="NY"` code, `region_name="New York"` name), but the seeded member stored the region CODE in `regionName` (data drift). **Fixed at source:** `seed-b2b-addresses.mjs` now maps `regionId=code, regionName=name` (via a US/CA code→name map) and normalizes existing addresses; reseeded vcst-qa (**11 addresses across 5 orgs** corrected). SR-GQL-036/039 assert the region **NAME** again (`@td(ORG_ACME.regionName)`="New York") → **PASS 6/6** live. `td:reconcile` check [9] now clean for all 15 seeded orgs.
...
3. **Previously-blocked — ALL RESOLVED:**
   - **SR-GQL-012/017/024 (SR_REP_LOCKED) — RESOLVED (was mis-diagnosed as "needs a browser").** A qa-backend-expert live investigation proved the locked-member exclusion IS headless-testable: login failed only because the rep's *default* org is the locked ACME; an **org-scoped grant** (`organization_id`=TechFlow + `storeId`) signs it in under its unlocked org, and the ACME membership lock still excludes it (confound-checked — it's the lock, not token scope). Wired `organization_id` into `graphql-auth.ts` + a `SR_REP_LOCKED_TFCTX` auth alias + `SR_REP_LOCKED_EMAIL`. All three cases now `[AUTH role=SR_REP_LOCKED_TFCTX]`, `Automated`, **PASS live** (012: ACME null in list; 017: `salesRepCustomer` clean null; 024: `salesRepOrders.totalCount=0`). Fixture/seed unchanged (ACME must stay the locked org — SR-GQL-007 depends on it).
   - **SR-GQL-008/013/027:** added `STORE_ID_SECONDARY=Electronics` to `.env.defaults` (verified live: `salesRepOrderStatuses(storeId:"Electronics")` → 200/8 statuses).
   - **SR-FE-005/017/018/019:** `SR_REP_PAGING` already seeded on vcst-qa (verified live: `salesRepCustomers.totalCount=12` as the paging rep; cases just target `agent-test-sr-paging@example.com`).
...
- [x] **Data cleanup — DONE.** `seed-b2b-addresses.mjs` regionName code→name fix + reseed corrected 11 addresses across 5 orgs; SR-GQL-036/039 reverted to assert the name (PASS 6/6). The ORG-003 "full-address drift" was a parity-check false positive (it read `addresses[0]`, a non-default Austin row; the actual default matches the CSV) — parity check now compares the default shipping address.
