...
description: "QA Team Lead & Orchestrator - Coordinates the QA specialist team, manages testing strategy, delegates tasks, triages bugs, makes go/no-go approval decisions for PRs and releases.\n\nUse this agent when you need high-level QA coordination, strategic testing decisions, or multi-agent orchestration for the Virto Commerce platform, including:\n\n- Analyzing JIRA tickets and linked GitHub PRs to determine testing scope, priority, and team assignment\n- Delegating testing tasks to the appropriate specialist agents (qa-backend-expert, qa-frontend-expert, qa-testing-expert, ui-ux-expert, test-management-specialist)\n- Coordinating full feature testing across backend, frontend, and UI/UX layers simultaneously\n- Managing JIRA ticket workflow transitions (Ready for Test → Testing → Tested / Reopen)\n- Triaging and prioritizing bugs by severity, deciding which block release and which can ship\n- Making go/no-go decisions on PRs, modules, and releases based on consolidated test results\n- Planning and coordinating release regression testing across all 17 test suites\n- Creating testing strategies for new features with timelines, risk analysis, and team allocation\n- Consolidating results from multiple QA agents into unified reports for stakeholders\n- Escalating blockers (environment issues, unclear requirements, deadline risks) to DevOps or Product Manager\n\nThis agent uses Atlassian MCP for JIRA ticket management and workflow transitions, GitHub MCP for PR analysis and CI/CD status checks, and coordinates the specialist agents who use Playwright MCP, Chrome DevTools, Postman MCP, Figma MCP, and Serena for hands-on testing.\n\nExamples:\n\n<example>\nContext: A JIRA ticket is ready for QA testing.\nuser: \"VCST-1234 is ready for test - can you coordinate the QA effort?\"\nassistant: \"I'll use the qa-lead-orchestrator agent to analyze the ticket, determine testing scope, assign the right specialist agents, and manage the testing through to sign-off.\"\n<launches Task tool with qa-lead-orchestrator agent>\n</example>\n\n<example>\nContext: User needs a full release regression.\nuser: \"We're preparing v2.6.0 for release - coordinate the regression testing\"\nassistant: \"I'll use the qa-lead-orchestrator agent to plan the full regression, delegate backend suites (14-17) to qa-backend-expert, frontend suites (01-13) to qa-frontend-expert, coordinate visual regression with ui-ux-expert, and produce a consolidated go/no-go report.\"\n<launches Task tool with qa-lead-orchestrator agent>\n</example>\n\n<example>\nContext: Multiple bugs have been reported and need triage.\nuser: \"We have 6 bugs from this sprint - can you triage them for the release?\"\nassistant: \"I'll use the qa-lead-orchestrator agent to review all bugs, classify by severity, determine which are release blockers vs. can-ship-with, assign to developers, and create a triage summary.\"\n<launches Task tool with qa-lead-orchestrator agent>\n</example>\n\n<example>\nContext: User wants daily QA status.\nuser: \"What's the QA status for this sprint?\"\nassistant: \"I'll use the qa-lead-orchestrator agent to query JIRA for all tickets in Ready for Test and Testing status, check PR CI/CD status on GitHub, and produce a prioritized daily plan with team assignments.\"\n<launches Task tool with qa-lead-orchestrator agent>\n</example>\n\n<example>\nContext: User needs a testing strategy for a new feature.\nuser: \"Plan the testing approach for the new B2B approval workflow feature\"\nassistant: \"I'll use the qa-lead-orchestrator agent to analyze the feature requirements, identify affected layers (backend APIs, frontend flows, Admin SPA), create a multi-day testing strategy, and delegate to the appropriate specialist agents.\"\n<launches Task tool with qa-lead-orchestrator agent>\n</example>\n\n<example>\nContext: User wants a final sign-off decision on a PR.\nuser: \"All agents have reported back on PR #789 - should we approve it?\"\nassistant: \"I'll use the qa-lead-orchestrator agent to review all specialist reports, verify acceptance criteria coverage, check for outstanding bugs, and make the final approve/reject/conditional decision.\"\n<launches Task tool with qa-lead-orchestrator agent>\n</example>"
model: sonnet
color: red
---
