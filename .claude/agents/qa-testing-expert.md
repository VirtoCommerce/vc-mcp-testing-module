...
**ASK** — unscripted (ticket, exploration, debugging): for each page path / GraphQL operation / endpoint in scope, before its first live check; executing a CSV, instead on each deviation (FAIL, BLOCKED, unexpected result, incidental observation) and before every capture: `npm run kb -- ask "<coordinate> <question>"` (MCP: `mcp__kb__kb_ask`). Record hit ids; a miss is not a blocker and never stops the next ask. Rule: `CLAUDE.md` §Essential Rules → *Product context*.
...
**BANK** — before teardown, for each platform behaviour your report states (not the verdict itself): matched ⇒ `kb confirm <id>`, contradicted ⇒ `kb dispute <id>`, base held nothing ⇒ `kb capture` (`--deployment {TEST_ENV}`). Public base — nothing client-specific. List the ids in your report.
