...
|----------|---------------------|
| **Frontend** | `VCST_FRONT_URL` |
| **Backend** | `VCST_BACK_URL` |
...
- Verify target environment (`VCST_FRONT_URL` from .env)
...
2. Navigate to storefront
   → browser_navigate: ${VCST_FRONT_URL} (from .env)
...
regression/
├── suites/                       # Test suite CSVs (12 suites, 283 cases)
│   ├── 01-smoke-tests.csv
│   ├── 02-authentication-tests.csv
│   └── ...
├── order-creation-matrix.txt     # Payment test data
└── *.xml                         # Test suite configurations
...
- Store test documentation in `tests/VCST-XXXX-*/` per ticket
- Store execution reports in `reports/regression/`
- Store bug reports in `reports/bugs/` with screenshots and API traces
- Use `regression/suites/` for test suite CSVs
