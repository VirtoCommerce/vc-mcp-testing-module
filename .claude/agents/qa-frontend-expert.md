...
description: "Frontend & Storefront QA Specialist - Customer-facing storefront, Admin SPA hybrid testing, user journeys, checkout flows. Reports to qa-lead-orchestrator."
model: opus
color: orange
---
...
- Use for: Explore storefront codebase, understand component structure
- When to use: Investigating bugs, understanding frontend implementations
- Key tools:
  - `get_symbols_overview` - View component structure
  - `find_symbol` - Locate specific components/functions
...
- Payment test data: `regression/order-creation-matrix.txt`
...
| `inventory/` | `vc-inventory.csv` | Stock levels, availability |
...
URL: https://[env]-store.virtocommerce.com
...
Steps:
1. From homepage, click on category (e.g., "Electronics")
   Expected: Category page loads, products display

2. Verify Product Listing Page (PLP):
   □ Category breadcrumb shows: Home > Electronics
...
1. LOG IN TO ADMIN SPA
   → Navigate to Admin panel (${VCST_BACK_URL} from .env)
...
Full report: /test-results/STORE-890/"
```
...
Environment: QA
Linked Issues: STORE-890 (parent feature)
