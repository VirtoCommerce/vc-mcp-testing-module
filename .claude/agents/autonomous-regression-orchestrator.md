...
> **REAL-USER RULE (propagate to teammates).** Every teammate you spawn MUST drive the browser like a customer — click/type/hover/scroll/wait — never `browser_evaluate` / `run_code_unsafe` / `evaluate_script` to bypass the UI (`hooks/enforce-real-user.mjs` blocks it). Before creating a JIRA ticket from a teammate failure, verify the repro is a real-user sequence; an API-only 4xx/5xx is NOT a UI bug. Full rule: `knowledge/agents/qa/shared-instructions.md` §Browser Interaction.
...
2. **Reuse check** — if `test-data/b2b/.seed-fingerprint.json` mtime < 2 hours old AND its `kind`/`env`/`storeId` match the profile, skip and log "Seed reused from {timestamp}".
