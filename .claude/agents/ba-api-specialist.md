...
- `.claude/knowledge/domain/functionality-map.md` — **Step 0, always** (`.claude/knowledge/agents/ba/shared-instructions.md` §Step 0): the prior BA analysis, prior test models and existing suites for this surface, per domain. Read it before assessing an API surface someone on this team may already have documented; it is an index of pointers, so it can never ground a claim.
