...
| `td:validate:pb-shared` (`cms/validate-pagebuilder-shared-components-data.mjs` → `cms/shared-components-specs.mjs`) | VCST-4933 Page Builder SHARED COMPONENTS (`npm run seed:pb-shared`, teardown `seed:pb-shared:teardown`). A **block-type** guard first, because that is how this fixture family dies silently: a page authored via REST with the capitalised block type `"Text"` is accepted at HTTP 200, is indexed, resolves through `slugInfo`, and renders a COMPLETELY EMPTY content area with **zero console errors** — measured 2026-09-16 on `/agent-test-q5-plain` (6 × `Text`), where it was very nearly filed as a Critical product bug against the ticket under test. Only the LOWERCASE types have renderers (`title|text|image|predefined-product-list`), so the guard FAILS any fixture block outside that set. Second, a VACUITY guard over the divergences the model depends on: every section text must be DISTINCT (equal texts make a reorder, a dropped section or a partial expansion unobservable), the master page must keep ≥ 4 adjacent sections (a 3-section adjacent selection has to leave an unselected one whose ORDER is checkable), `SC_UNUSED` must reach usage **0** and `SC_USED` usage **> 0** (deletion is TERMINAL — the delete-succeeds and delete-blocked cases each consume their own component, so they can never share one), `SC_MULTI` must be used on ≥ 2 pages, and the A/B CONTROL page must reference **zero** components (it is what makes any "does not render" claim decidable). Third, the ROLE contract: seven back-office roles whose page-builder base is IDENTICAL and which differ ONLY on `builder:shared-components:{read,create,update,delete}` — if the base differed too, a 403 could not be attributed to the shared-components gate; exactly three are "all four minus one", one holds all four, one holds none, and one holds all four with a MISMATCHED `user.StoreId` (the BSC-3 control, whose uniform 403 is CORRECT behaviour, not a defect). Plus the ordinary hygiene: `AGENT-TEST-` prefixes, unique keys, no runtime GUID/32-hex component id in the spec or the committed content fixture, all 17 aliases registered, and passwords only as `{{PAGEBUILDER_SC_USER_PASSWORD}}`. Live companion: the seeder’s own `--verify`, which fails the run when a published permalink does not DELIVER rendering block types through the storefront `slugInfo`→`pageDocument` pair, when a component’s live `usageCount` disagrees with the usage DERIVED from the page fixtures, or when any of the seven accounts cannot obtain a token. |
| `td:validate:sales-rep-tasks` (`sales-rep/validate-sales-rep-tasks-data.mjs` → `sales-rep-tasks-specs.mjs`) | VCST-5732 Sales Rep TASK fixtures (`npm run seed:sales-rep-tasks`, teardown `seed:sales-rep-tasks:teardown`). A **VACUITY** guard first: it FAILS when the four date/completion group sizes stop being MUTUALLY DISTINCT (equal sizes let a filter wired to the wrong predicate return a plausible count and PASS), when any group empties, when the COMPLETION STRADDLE loses either direction (one completed task due TODAY *and* one due in the FUTURE — together they are the only thing proving completion outranks the date; both in the past would make "completed" and "past" one partition), when name order / due-date order / creation order stop being mutually different (a sort control that silently does nothing becomes undetectable), when the ONE shared calendar day (+7) stops carrying two tasks or any of the six deliberately EMPTY days after it (+8..+13) gains one (multi-task-day and empty-day indicators become indistinguishable), or when priority/type start CORRELATING with the date group. Second, the platform contract the server will not enforce for you: `type` is **NOT server-validated** (any string is accepted at HTTP 200 and then matches no filter), so the 8 legitimate `salesRepTaskTypes` values are asserted here; `priority` must stay within the server-validated High|Normal|Low. Third, the **relative-date** rule — every due date is a `dueOffsetDays`/`dueHourUtc` offset recomputed at seed time, and a literal date in the spec fails the guard (a literal is a hardcode with a delay fuse). Fourth, the ordinary hygiene: all 15 aliases registered `_inline` with `_notes`, business fields matching the spec EXACTLY, runtime fields (`id`, `due_date`, `seeded_at`, `decays_at_utc`, `owner_rep_email`) EMPTY in the committed base, no GUID leak (DV-021), no credential literal in the spec module or the aliases, and `SR_TASK_GROUPS`'s group-size EXPECTATIONS equal to the sizes DERIVED from the spec rows — so a case reading `@td(SR_TASK_GROUPS.open_today)` can never assert a count the seeder does not create. **DECAY — the one thing no static guard can catch:** the due dates are wall-clock relative, so the three `dueOffsetDays: 0` tasks stop being "due today" at the next UTC MIDNIGHT and 2/3/5/4 silently becomes 5/0/5/4. The warning is carried in the seeder header, the spec header and every alias's `_notes` (the guard fails if `SR_TASK_GROUPS._notes` loses it), and **any suite asserting group membership or size must re-seed first**. ISOLATION is **per REP ACCOUNT**, never bare: tasks are private to their owner (a second rep reads `totalCount: 0`), so two suites on the same rep see each other's mutations. Transport is the SCOPED `/graphql/sales-rep` endpoint as the owning rep (`SALES_REP_EMAIL` in the committed `.env.<env>`; `SALES_REP_PASSWORD` per-env-suffixed in the gitignored `.env.local`) — there is no admin REST surface for tasks. |
| `td:validate:sales-rep-demo` (`sales-rep/validate-sales-rep-demo-data.mjs` → `sales-rep/sales-rep-demo-specs.mjs`) | The sales-rep **DEMO** dataset (`npm run seed:sales-rep-demo`, teardown `seed:sales-rep-demo:teardown`), presentation-grade data that inverts the usual fixture contract: it carries deliberately NO `AGENT-TEST-` prefix, so the properties that replace the one-line keyword teardown are what this guard protects, and each fails SILENTLY when broken (the seed still runs, the data still appears, and the damage surfaces only in front of the audience the demo was for). Calls the spec module's own `demoProblems()` rather than reimplementing it — the seeder aborts on the same contract before any write, so this guard and the seeder can never disagree about "wrong". It FAILS on: any presentation leak (an `AGENT-TEST` string or a raw GUID in a name/email/summary a viewer can see), the two non-vacuity gaps (`DORG-CONTOSO` no longer served by both reps, so the multi-rep surface is undemonstrable; or two organizations sharing a `productSearch` phrase, so their order line items could not stay disjoint); a GUID literal in the spec module, the seeder, or a committed demo data file (runtime ids live in `test-data/aliases.<env>.json` only, DV-021) or the demo id ledger (`_demo_sales_rep_ledger`) committed into the base `aliases.json` instead of the per-env overlay; a missing `DEMO_DOCUMENTS[].sourceFile` under `test-data/uploads/` (WARN only — the seeder falls back to generated bytes); a demo buyer password passed to `resolvePassword()` as a literal instead of a `{{VAR}}` token; `EXCLUDED_PRODUCT_RE` failing to reject the `AGENT-TEST-*` products actually live in the catalog, or the seeder no longer calling `isDemoSafeProduct` at discovery; and the ledger key losing its `_` prefix, which is the only thing keeping `td:reconcile` check [11] from member-probing every order/file GUID the ledger holds and reporting a wave of false STALE rows. |
