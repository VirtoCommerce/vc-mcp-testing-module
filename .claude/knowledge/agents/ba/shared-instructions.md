...
**But a doc page is authoritative for MECHANISM, not for SURFACE.** Exact UI strings, control types, layout
and counts are `{OBSERVED}` claims, never `{DOC}` — a user guide paraphrases labels by design; a documented
rule binds only to the surface the doc names; and docs contradicting an existing artifact is a trigger to
observe, not a licence to overwrite it. This binds the `BL-*`/`ECL-*` write path too, where a doc-sourced
invariant is promoted. Full rule, the enforceable quote test and the measured incident:
