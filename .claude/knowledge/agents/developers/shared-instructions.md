...
**Those three tell you what the code SHOULD do. `kb_ask` tells you what it WAS SEEN doing**, on a named deployment — which is the half of a bug report that usually is not written down, and it lands between steps 1 and 2 (`npm run kb -- ask "<q>"` needs no MCP server, so it works in a fresh `.fix-workspace/` checkout). Put the coordinate in the question — endpoint, GraphQL operation, page path. **It fires on the coordinates, not on a suspicion:** every page path, GraphQL operation and endpoint the task's input names — suite CSV, ticket, diff — is a question, asked before its first live check, *including while you execute prescribed steps*, which is where most observations are made and where a "do I know this?" trigger never fires ([`../authoring-standard.md`](../authoring-standard.md) §5.1). Exit 1 = nobody wrote it down; what you then had to establish to build the fix is worth a `kb_capture`, because the verifier at Gate 6 is about to need exactly it. **Contradicts an entry ⇒ `kb_dispute`, matches ⇒ `kb_confirm`.** The base is PUBLIC — never put client code, customer names or credentials in one. Full rule: [`../../../CLAUDE.md`](../../../CLAUDE.md) §Essential Rules → *Product context*.
