...
**That order is for what the platform SHOULD do. For what it DOES, `kb_ask` comes before step 3 and before grepping an oracle** — it is the banked result of a live check somebody already ran, on a named deployment, with a trust count (`npm run kb -- ask "<q>"` when the MCP server is not bound yet; it needs no server). **Put the coordinate in the question** — endpoint, GraphQL operation, page path. **It fires on the coordinates, not on a suspicion:** every page path, GraphQL operation and endpoint the task's input names — suite CSV, ticket, diff — is a question, asked before its first live check, *including while you execute prescribed steps*, which is where most observations are made and where a "do I know this?" trigger never fires ([`../authoring-standard.md`](../authoring-standard.md) §5.1). Exit 1 = nobody wrote it down: find out, then `kb_capture`. **Contradicts an entry ⇒ `kb_dispute`, matches ⇒ `kb_confirm`** — capturing it again instead gives the base two answers to one question, and v1 cannot merge them. The base is PUBLIC. Full rule and why: [`../../../CLAUDE.md`](../../../CLAUDE.md) §Essential Rules → *Product context*.
