...
- **Rule:** At token issuance the active organization is resolved by a **fixed, ordered chain**, and every step is constrained to organizations the user can actually access:
  ...
  **Accessible** is a two-axis predicate over (userId, orgId): `NOT IsCurrentlyLocked` **AND** the membership's effective status (BL-B2B-013) is not a blocking status (`Invited` / `Rejected` / `Deleted`). An org with **no membership row** is not excluded by that row's absence — the effective status falls through to the contact's own status. A user whose membership list is empty, or whose userId is absent, is treated as having **all** their associated orgs accessible (the filter is skipped, not applied as a deny).
...
- **Verify:** Multi-org user, `password` grant naming org Y explicitly → active org is Y even when the contact's `CurrentOrganizationId` is X (step 1 beats step 3). Switch orgs through the real UI, then sign out and sign in again → the resend/persisted org is honoured, not the previous first-accessible one. Put a single-org user's only membership at a blocking status → sign in with **no** explicit `organization_id` → the org is absent from the resolved context (org switcher and org-scoped navigation are gone), not merely refused. Confirm the switcher lists only accessible orgs.
- **Violation signal:** A blocked or blocking-status org is selected as active; `DefaultOrganizationId` is pinned despite being inaccessible; an explicit `organization_id` is ignored in favour of the contact's stored org; a user with associations but no membership rows is denied every org (the skip-the-filter branch inverted into a deny).
- **Agents:** qa-backend-expert (token endpoint, memberships), qa-frontend-expert (org switcher, post-login context)
- **Source:** `vc-module-customer` `OrganizationAccessResolver.cs` (`ResolveOrganizationIdAsync` = steps 3–5; `GetAccessibleOrganizationIdsAsync` = the two-axis predicate and the empty-userId/empty-list skip) + `OrganizationIdRequestValidator.cs` `GetOrganizationId` (steps 1–2). Live-confirmed on the environment: step-1 precedence (an explicit org in the grant won over the stored one), the blocking-status exclusion (a `Rejected`-membership single-org user resolved to no organization), and the switcher listing only accessible orgs. The internal ordering **between** steps 3 and 4 is source-only — it was not isolated live. Docs axis: N/A — the published guides describe multi-organization assignment and switching but no resolution order or accessibility predicate; the behaviour shipped in the same change being audited (waived).
...
- **Verify:** Single-accessible-org user, membership both locked and at a blocking status → `password` grant naming that org → exactly one error, code `user_is_locked_in_organization`; unlock → the same call returns the status code instead; re-lock → back to the lock code. Multi-org user with one blocked org → `password` grant naming the blocked org → **HTTP 200**, active org is the accessible one (no error). Same request as a non-`password` grant → refused. Single-org user at a blocking status signing in with **no** explicit org → HTTP 200, session has no organization.
