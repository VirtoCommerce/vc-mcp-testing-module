...
> Reference file for ui-ux-expert agent. Read when performing UX evaluations. Heuristic findings should be cross-referenced against the `BL-*` invariants (`knowledge/oracles/business-logic.md`) and the the edge case library (`knowledge/oracles/e-commerce-edge-cases-library.md`) — many UX issues map to a concrete invariant or ECL pattern, and citing the link makes the finding filable as a bug instead of a vague complaint.
...
|-----------|---------|-------|
| #1 Visibility of system status | `BL-UI-001` (no layout shift hides "what changed"), `BL-CART-002` (stock-mid-session), `BL-ORD-*` (status feedback) | Missing loading state on Place Order = #1 violation AND BL-ORD risk |
...
| #3 User control | `BL-CART-*` (cart edit, remove), `BL-CHK-*` (back/edit during checkout) | "Cannot edit shipping after picking method" = #3 + BL-CHK |
| #4 Consistency | `BL-UI-002`, `BL-UI-005` | Spacing grid drift, alignment drift, color-meaning drift across pages |
| #5 Error prevention | `BL-CART-001` (max qty), `BL-CHK-*`, `ECL-2.*` (input validation edges) | Accepts negative qty = #5 + BL-CART |
...
| #8 Aesthetic / minimal | `BL-UI-004` (overflow), `BL-UI-005` (hierarchy implies alignment) | Two competing CTAs, overflowing modals |
| #9 Error recovery | WCAG 3.3.1 (error identification), WCAG 3.3.3 (error suggestion), `ECL-2.*` | "Invalid input" without specifics = #9 + WCAG 3.3.1 |
