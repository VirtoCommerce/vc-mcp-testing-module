...
> Reference file for ui-ux-expert agent. Read when validating that a component honors the active design system. Canonical invariants: `BL-UI-002` (spacing grid) and `BL-UI-005` (alignment). Canonical helper: [`scripts/lib/measure-layout.ts`](../../../scripts/lib/measure-layout.ts). Canonical suite: none — `048b-layout-stability.csv` was removed 2026-07-25; these checks now run only through this skill.
...
**Cross-check against `knowledge/automation/storefront-config-flags.md`** for active preset → expected token shifts.
