...
> **Terminal command:** `/qa-design <component | page | flow>` — see [commands/qa-design.md](../../commands/qa-design.md). The command parses the argument, resolves audit scope from `knowledge/oracles/critical-ui-scope.md`, and dispatches `ui-ux-expert`; this file is the methodology library it consults.

Validate design system consistency and run UX heuristic evaluations against the active Coffee theme tokens, the BL-UI invariants, and Nielsen's 10 usability heuristics.
...
- `knowledge/oracles/business-logic.md` Domain 15 (BL-UI-001..006) — the canonical UI invariants. A violation is a FAIL regardless of how the result looks.
...
- ~~`048b-layout-stability.csv`~~ — **removed 2026-07-25.** No suite drives BL-UI-001..006 against the live storefront any more; this skill is now the only executor of those invariants. Work from the audit protocols + coverage matrix in `knowledge/oracles/critical-ui-scope.md` (all cells `GAP`).
- `knowledge/automation/storefront-config-flags.md` — active theme preset + flags affecting which tokens render.
...
| `KNOWN_DIVERGENCE` | the spec declares this rule unshipped in code | advisory — **never a FAIL, never a clean PASS** |
...
- **Design spec diff table** — always present (the axis is default-on): one row per checked token / control / icon / stroke bucket with `CONFIRMED / DRIFT / MISSING / UNSPEC / KNOWN_DIVERGENCE`, the spec value vs the live value, and the artboard path it came from. Header carries the `summarizeDesignFindings` one-liner, the `unresolved` count, and **which source fed the expectations** (extractor output vs. values relayed by hand — a relayed run has unknown, not zero, unresolved entries and partial parity coverage). When the source could not be reached the table is replaced by a single `SKIPPED — <reason>` line; it is never omitted and never reported as PASS
