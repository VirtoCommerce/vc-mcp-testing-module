# The pre-flight axes — one mechanism, four instances

Single source of truth for the derived tokens `1b` resolves: **`layer` (2b) · `visual_surface` (2c) ·
`contract_surface` (2d) · `coverage_surface` (2e)**. The command carries one table; this file carries the
...
| | `layer` (2b) | `visual_surface` (2c) | `contract_surface` (2d) | `coverage_surface` (2e) |
|---|---|---|---|---|
| **Unresolved ⇒** | **`null` + `UNRESOLVED`** | `true` | `true` | `true` |
| Shape | 6 values + `cross-layer` | boolean | boolean | boolean |
| Dispatches an agent? | no | **yes** (`ui-ux-expert`) | no | no |
| Costs I/O in `1b`? | no | no | **yes** (~8.6 s) | yes (~1 s, wave B) |
| Has a conflict rule? | **yes** (`layers_conflict`) | no | no | no |
| Consumed by | 5f / 5h routing | Step 4's visual lane | `1c`/`1d`/`1e`/3b pack | Step 2a's dispositions |
