...
> **Before writing a new query/mutation**, check the curated fixture library at [test-data/graphql/index.json](../../../test-data/graphql/index.json). It indexes every schema-validated `.graphql` file under `queries/` and `mutations/` (`totalFixtures` is the count) with headers declaring required-vars, optional-vars, `gqlVars`, and `exampleVars` — see [test-data/graphql/README.md](../../../test-data/graphql/README.md) for the full use-and-add contract. Reuse a fixture path in your test case rather than re-typing the query body. Add new queries by dropping a `.graphql` file in [test-data/graphql/queries/](../../../test-data/graphql/queries), registering it in `index.json` (and bumping `totalFixtures` — neither is gate-enforced), then running `npm run graphql:fixtures:validate`. The schema reference in [knowledge/api/graphql-schema.md](../../knowledge/api/graphql-schema.md) has the full reuse policy.
