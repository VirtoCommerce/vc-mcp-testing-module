...
1. **Understand the seam** (route/page → component → composable → store → GraphQL → util). Confirm the
   ...
   not a code bug → BAIL to G0). See `.claude/agents/knowledge/architecture/vc-frontend-architecture.md`.
...
- `.claude/agents/knowledge/oracles/vc-bug-catalog.md` — don't re-introduce a historical VC-UI-* failure pattern
