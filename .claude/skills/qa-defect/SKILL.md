...
4. **For verify:** Fetch ticket + linked PR via Atlassian MCP → confirm fix is deployed → execute original STR verbatim (must pass 3 consecutive times) → run 2-3 adjacent regression checks → check for side effects (console errors, network failures) → decision: all pass → transition to TESTED and **stop there** (a QA flow never sets `DONE` — `.claude/knowledge/execution/ticket-status-transitions.md` §9.5); any fail → transition TESTED → REOPEN with new evidence.
