...
6. **Output:**
   ...
   - **Locate every finding precisely** — never fabricate a source location. Pin each to the first available of: `data-test-id` (see `knowledge/automation/storefront-selectors.md`) → `role` + accessible name → DOM path / tree position. Use the tag/role/label the keyboard walk already captured; if a finding can't be located, say so ("located by selector only").
