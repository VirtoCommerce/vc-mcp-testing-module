...
```bash
node skills/qa-local-env/resolve-task.mjs VCST-5173
node skills/qa-local-env/resolve-task.mjs VCST-5173 --gen --print   # also run gen-manifest
```
...
node skills/qa-local-env/gen-manifest.mjs --print

# with task augmentation (flags from the resolve step)
node skills/qa-local-env/gen-manifest.mjs \
...
```powershell
pwsh -File skills/qa-local-env/provision.ps1 -Action up `
...
```bash
node skills/qa-local-env/resolve-theme.mjs latest        # newest GA release (vc-theme-b2b-vue-x.y.z)
node skills/qa-local-env/resolve-theme.mjs alpha          # newest alpha (…-alpha.NNNN — what vcst-dev runs)
node skills/qa-local-env/resolve-theme.mjs pr 2350 --url  # newest build of a specific PR
```
...
```bash
node skills/qa-local-env/healthcheck.mjs --token   # password auto-resolves (see below)
# task probe (VCST-5173): confirm the redesign fields exist in the live schema (no data/token needed)
node skills/qa-local-env/healthcheck.mjs \
...
node skills/qa-local-env/healthcheck.mjs --token \
...
node skills/qa-local-env/healthcheck.mjs --token --module-errors
```
...
```powershell
pwsh -File skills/qa-local-env/provision.ps1 -Action stop     # pause: containers down, files + cache kept
pwsh -File skills/qa-local-env/provision.ps1 -Action clean    # stop + wipe data volumes (fresh DB next start)
pwsh -File skills/qa-local-env/provision.ps1 -Action remove   # destroy containers + volumes + temp files
```
