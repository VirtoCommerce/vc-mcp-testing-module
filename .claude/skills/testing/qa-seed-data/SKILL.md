...
`seed-test-data.js` covers only `minimal/catalog/full/teardown` — **all company users (B2B orgs/contacts/logins/memberships + personal + impersonation + loyalty + white-labeling) are `seed-company-users.mjs`**, and pricing is folded into the `catalog`/`full` profiles. All configurable products are now seeded by the single `seed-configurable.mjs` (families `base`/`conditional`/`default`/`bike` selectable via `--group`); it imports `seed-common.mjs` and keeps each family's original catalog/category/pricelist names for idempotency.
