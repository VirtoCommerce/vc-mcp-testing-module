...
Pick the path by what you're seeding. Both make `@td()` resolve afterward, but by different means: **script seeders (Path A) persist runtime GUIDs automatically** to `test-data/aliases.{env}.json` (see §Write-back model below) — no manual step; **Postman (Path B)** requires writing the captured IDs back into `test-data/` yourself (Step 6).

### Path A — Repo seed scripts (direct REST/xAPI) — the practical default

Node seeders in [`scripts/seed-data/`](../../../../scripts/seed-data/) read CSVs from [`test-data/`](../../../../test-data/), call the platform API directly, are **idempotent** (look-up-then-create), respect `TEST_ENV` (via `config.js` / `seed-common.mjs`, with per-env `_${ENV}` secret-suffix promotion), are gated by **`ENV_RISK`** (config, not hostname — blocks `production`, runs on any dev/test/staging/localhost/customer env), and **persist runtime platform GUIDs to `test-data/aliases.{env}.json`** (see §Write-back model). No standalone `_seed-results-*.json` reports (only `seed-test-data.js` keeps one, for teardown id-recovery). No Postman/Newman needed.

**§Write-back model (VCST — seed writeback).** Script seeders resolve/create entities, then persist only the **drifting runtime GUIDs**; business keys / SKUs / codes stay in the committed CSVs.
- Non-`vcst` env → GUIDs written to `test-data/aliases.{env}.json` (`localhost` gitignored; `vcptcore`/`virtostart`/customer envs committed) via `seed-common.mjs` `syncEnvAliases()` / `writeEnvAliasOverride()`. The resolver layers this file field-by-field over `aliases.json`.
- Primary `vcst` → not auto-written; canonical GUIDs stay curated in `aliases.json` (+ `users.csv` `platform_id`, refreshed in place by the b2b writeback).
- Static business-key aliases (`COUPON_*`, `STORE_*`, `FC_*`, `PRICELIST_*`, `PROD_*`, `WL_*`, `BOPIS_*`) need no writeback — their ids are stable CSV keys.
- Regression seed-reuse freshness is tracked by `test-data/b2b/.seed-fingerprint.json` (gitignored stamp), not a results report.
...
| [`seed-standard-products.mjs`](../../../../scripts/seed-data/seed-standard-products.mjs) | 6 standard products (`PROD_HEADPHONES`, `PROD_LAPTOP`, `PROD_OOS`, `PROD_LOW_STOCK`, `PROD_PACK_SIZE`, `PROD_TIER_PRICED`) | `node scripts/seed-data/seed-standard-products.mjs [--dry-run] [--only PROD-001]` | — (`PROD_*` resolve by SKU) |
| [`seed-configurable-products.mjs`](../../../../scripts/seed-data/seed-configurable-products.mjs) | Single-section configurable products (CFG-012/014/015/016/018) + child-option products | `node scripts/seed-data/seed-configurable-products.mjs [--dry-run] [--only CFG-012]` | `aliases.{env}.json` (id + configuration_id) |
| [`seed-conditional-sections-extended.mjs`](../../../../scripts/seed-data/seed-conditional-sections-extended.mjs) | Conditional-cascade CFGs (CFG-022..029, `dependsOnSectionId`) | `node scripts/seed-data/seed-conditional-sections-extended.mjs [--dry-run]` | `aliases.{env}.json` |
| [`seed-default-option-cfg.mjs`](../../../../scripts/seed-data/seed-default-option-cfg.mjs) | Default-option CFGs (VCST-4715: CFG-030/031, `option.isDefault`) | `node scripts/seed-data/seed-default-option-cfg.mjs [--only CFG-030]` | `aliases.{env}.json` |
| [`seed-bike-flat-cfg.mjs`](../../../../scripts/seed-data/seed-bike-flat-cfg.mjs) | Flat Bike CFG-032 for suite 072 (CFG-PDP-*) | `node scripts/seed-data/seed-bike-flat-cfg.mjs` | `aliases.{env}.json` |
| [`seed-promotions.mjs`](../../../../scripts/seed-data/seed-promotions.mjs) | Marketing promotions + rewards + coupons from `test-data/promotions/*.csv` | `npm run seed:promotions` (`[--dry-run] [--only P01] [--teardown]`) | — (static keys) |
| [`seed-bopis.mjs`](../../../../scripts/seed-data/seed-bopis.mjs) | BOPIS pickup locations from `test-data/stores/bopis-locations.csv` (vc-module-shipping; linked to an existing FFC) | `npm run seed:bopis` (`[--only LOC-001] [--teardown]`) | — (static keys) |
| [`seed-catalog-properties.mjs`](../../../../scripts/seed-data/seed-catalog-properties.mjs) | Catalog/variation property definitions + dictionary values from `test-data/catalogs/properties.csv` | `npm run seed:properties` (`[--only PROP-001] [--teardown]`) | — |
| [`seed-white-labeling.mjs`](../../../../scripts/seed-data/seed-white-labeling.mjs) | Menu link lists + white-labeling org config + users from `test-data/white-labeling/*.csv` | `npm run seed:white-labeling` (`[--skip-users] [--teardown]`) | — (static keys) |
| [`seed-company-users.mjs`](../../../../scripts/seed-data/seed-company-users.mjs) | **ONE script for every company user** (VCST-5406 — replaces the old `seed-b2b-fixtures` / `seed-users` / `seed-impersonation-targets` / `seed-loyalty-users`). CSV/env-driven over [`scripts/lib/user-provision.mjs`](../../../../scripts/lib/user-provision.mjs). Kinds: `all` (default) · `b2b` (orgs parent-child + contacts + org-scoped logins + cross-org memberships) · `imp` (ORG-009…019 + blocked/invited targets, status-driven) · `cross-org` (organization-memberships.csv) · `personal` (env customer roles + agent-pool + test-users) · `loyalty` (VIP/Wholesale group). Account status (Approved/Locked/EmailUnconfirmed) is data-driven from the users.csv `status` column. **One unified `--teardown`** sweeps ALL sources (no orphan gap). | `npm run seed:company-users` (`b2b` / `personal` / `imp` via `seed:b2b` / `seed:users` / `seed:impersonation`; `--teardown` / `--dry-run`) | `aliases.{env}.json` (platform_id); vcst→users.csv |
...
`seed-test-data.js` covers only `minimal/catalog/full/teardown` — **all company users (B2B orgs/contacts/logins/memberships + personal + impersonation + loyalty) are `seed-company-users.mjs`**, and pricing is folded into the `catalog`/`full` profiles. The CFG `.mjs` seeders are phased (Phase 1+2 → conditional → default-option → bike) and share catalog/category/pricelist/ffc/virtual-catalog wiring.
...
> personal + impersonation + loyalty across all sources; `seed:b2b:teardown` / `seed:users:teardown` are
> back-compat aliases of it), `seed:pricing:teardown`, `seed:inventory:teardown`,
...
- **Script seeders (Path A) write runtime GUIDs automatically** to `aliases.{env}.json` (see §Write-back model) — no manual step. **For Postman (Path B)**, after the run write the captured entity IDs back into [`test-data/`](../../../../test-data/) (CSVs referenced by `aliases.json`) so downstream suites resolve them via `@td()` — see [`qa-postman/test-data-fixtures.md`](../qa-postman/test-data-fixtures.md) for the resolver contract
