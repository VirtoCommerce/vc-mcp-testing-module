...
```bash
node .claude/skills/project-init/scaffold-env.mjs --env myqa --tracker jira --client-vcs github --print
# Azure Repos client on Jira → ADO_ORG/ADO_PROJECT are emitted for the code host too:
node .claude/skills/project-init/scaffold-env.mjs --env acme --tracker jira --client-vcs azure-repos --print
```
...
```bash
TEST_ENV=<env> node .claude/skills/project-init/discover-repos.mjs \
...
```bash
TEST_ENV=<env> node .claude/skills/project-init/derive-context.mjs \
