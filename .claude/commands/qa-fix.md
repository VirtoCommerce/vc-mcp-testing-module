...
> (`moduleFrontendSubApps` in `ci/config/fix-repos.json`, e.g. `vc-module-pagebuilder`'s Vue 3 "shell" at
> `src/VirtoCommerce.PageBuilderModule.Web/Apps/page-builder-shell/`). When Gate 1's RCA anchor falls under a declared sub-app path, the
> developer agent for **this bug** is `fullstack-frontend` instead (via `/vc-shell-fix`), scoped to the
> sub-app directory — `repoKind` itself stays `"module"`, unaffected for every other bug in that repo.
