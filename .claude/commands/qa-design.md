...
- **Component name** — matches an entry (case-insensitive) in `knowledge/oracles/critical-ui-scope.md` Component Inventory (VcButton, VcProductCard, VcLineItem, VcTable, VcDialog, Popover, VcSidebar, BOPIS Modal) → in-matrix component.
...
- Read the relevant matrix in `knowledge/oracles/critical-ui-scope.md` (components matrix for component, pages matrix for page).
...
1. **Resolve selector + render-location:**
   - Check `knowledge/automation/storefront-selectors.md` for the component name. Many vc-frontend components are documented there even if not in the critical matrix.
...
3. **Warn + offer to promote** — print: "`{Component}` is off-matrix. Audited with heuristic invariant scope: [list]." After audit, if findings exist or the component appears in audit history ≥ 3 times, suggest: "Consider adding to `knowledge/oracles/critical-ui-scope.md` so it's regression-protected." Never auto-edit the matrix.
...
1. **Find where the component is used across the storefront** — produce a confirmed candidate list, then pick up to 3 contexts.
   ...
   **Step 1.3 — Cross-check against the curated map** — read `knowledge/oracles/critical-ui-scope.md` Render-Location Map. For in-matrix components, the curated locations are high-confidence; treat them as candidates first, then add any GitHub-discovered locations the curated map missed (and surface the gap in the report).
   ...
   **Step 1.4 — Live probe to confirm rendering** — for each candidate URL, before audit:
   ...
   - Run `document.querySelectorAll('{selector}').length > 0` (selector from `knowledge/automation/storefront-selectors.md` or matrix row).
...
- Reference paths the agent must consult:
  - `knowledge/oracles/business-logic.md` § Domain 15 — BL-UI invariant definitions.
  ...
  - `knowledge/automation/storefront-selectors.md` — verified DOM selectors.
