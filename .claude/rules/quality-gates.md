...
| **G2** | Reproduction (red) | `fullstack-backend` (interactive) / `fix-*-agent` (CI) | A **new** unit test encodes the STR/RCA and **fails on current code** (xUnit for C#; vitest for vc-frontend; module Admin UI has NO in-repo JS harness — red→green proven via an uncommitted Node scratch harness in `.fix-workspace/_scratch/`, evidence in PR body, see `/angular-admin`). OR trivial-skip justified (one-line null-guard / typo / off-by-one, no behavioral branch) — noted in PR body | Cannot encode in code → `FIX_STATUS: FAILED`, escalate. |
...
| **G5** | Build + unit CI + SonarCloud QG green | orchestrator (poll) | Repo build + unit tests pass — locally first (`repoProfile.buildCmd` + `testCmd` from `ci/lib/repo-router.ts`), then the **PR's GitHub Actions** = all checks `success` + `mergeable`. **The product repos run a SonarCloud quality gate** (the `test-and-sonar` / `SonarCloud Code Analysis` check) — it MUST be green: no new bugs / vulnerabilities / unreviewed security hotspots, and **new-code** coverage + duplication within the project's QG thresholds | Code-fail → self-correct in the same repo (≤2 iterations), re-push, re-poll. **Sonar QG red → fix the valid findings on the changed lines** (real bug / vulnerability / security-hotspot, or add coverage for the new code) within the minimal-diff + never-edit-existing-tests rules; do NOT chase pre-existing debt or style nitpicks unrelated to the diff (mark them won't-fix in Sonar or note in the PR). Infra-fail / QG threshold owned by repo config → escalate (don't loop). |
