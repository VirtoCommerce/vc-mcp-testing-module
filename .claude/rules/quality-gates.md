...
| **G1** | Single target repo | orchestrator (validated by `isAllowedRepo`) | Resolves to **exactly one** allowed repo: a `vc-module-*` (C# and/or its Admin SPA UI), `vc-module-x-*` (xAPI), `vc-platform`, or `vc-frontend`. Owning layer ∈ {REST L4, xAPI L3, Admin-SPA L2 in the module repo, storefront L1 in `vc-frontend`} → kind (`module`/`platform`/`frontend`) picks the dev agent | **STOP.** Multi-repo / cross-module / off-allowlist → comment + hand off (`ROUTE_REPO` rejected). Never guess. |
...
| **G3** | Fix correctness (green) | kind-routed dev — `fullstack-backend` / `fullstack-frontend` / `fix-*-agent` | Minimal diff turns the repro test **green**; **ALL pre-existing tests (and vc-frontend stories) stay green and UNMODIFIED**; BL-* / BL-UI invariants preserved; no `vc-bug-catalog` regression re-introduced. **Admin SPA layout/CSS:** the fixed **render-harness screenshot** (rendered + verified in a browser by `qa-backend-expert`, **before the PR opens**) shows correct layout matching the canonical sibling blade; the diff uses only platform classes (`admin-spa-ui-conventions.md`), no inline `position`/fixed-px/`ng-style` height. Dev↔QA iterate ≤2× + squash | An existing test breaks → contract conflict → **STOP**, do NOT edit the test, report (human decision). |
