...
- `domain/functionality-map.md` — **GENERATED (`npm run map:refresh`, gated by `map:check`); MUST be read before analysing or designing against any surface.** It answers **two** questions. (1) *Where is the prior art* — per manifest domain: the suites and their measured `BL-*`/`ECL-*` citations, the domain-knowledge docs, **the prior BA analysis**, **the prior test model**, and the tickets already tested there, each **dated**. (2) *What IS this object* — a **`Test object`** block per domain: **purpose** (the value chain), **operations** (from the schema doc's own vocabulary, measured against the domain's suites), the **data** its assertions read (`@td()`/`{{VAR}}`), the **variants** that change behaviour without changing code (config flags, store defaults), the **constraints** with what a violation costs (BL severity tags, read never inferred), and **reverse edges**. The second half is the one that lets a tester design an experiment rather than walk screens — the measured Loyalty Missions failure. **`UNDECLARED` is a finding**: purpose and reverse edges exist only in a Test Model Part 0, so a domain with no model says so (measured: 1 of 13 domains has a declared purpose), and it is never filled with a guess. It exists because that prior art had no reader: 47 deliverables in `reports/ba/` were covered by one line telling one agent to *"skim"* them for dedup, and `reports/ba/test-models/` was durable specifically so the next ticket on a surface could reuse the model while nothing ever opened one (VCST-5346 has two). Consumed by `/qa-test` `1b` item 2-map (before `1c`/`1d` dispatch, so the brief carries paths rather than an instruction to go looking) and by the whole BA team at `knowledge/agents/ba/shared-instructions.md` §Step 0. **Three limits:** it is an index of pointers and **carries no behaviour**, so it can raise *"this was already analysed"* and can never ground an assertion as `{DOC}` (the release-ledger rule); a domain with no prior analysis says so, and that is a real gap rather than a rendering artifact; and a document it could not place from path + title is listed in §5 **unplaced, not unimportant**. Amend what it names — never fork a second file for one surface.
...
| **ui-ux-expert** | sonnet | Storybook component testing, WCAG 2.2 AA accessibility, design system, and the **`vs. DESIGN` axis** — diffing declared tokens / control geometry / icon name→glyph parity against a Claude Design project (`DesignSync` → `scripts/lib/verify-design-spec.ts`, methodology `skills/qa-design/claude-design-verification.md`). Runs by default against the project **the ticket's own Prototype link names** (no global default — `DESIGN_SYSTEM_PROJECT_ID` removed 2026-09-03; no design link ⇒ `SKIPPED`); precedence `BL-UI invariant > design spec > UX heuristic`; reports `SKIPPED`, never PASS, where `/design-login` is unavailable (web sessions, CI), and `KNOWN_DIVERGENCE` — advisory, never filed — for a mismatch the spec itself declares unshipped |
