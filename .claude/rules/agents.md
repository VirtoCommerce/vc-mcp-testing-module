...
>
> **ROOT CAUSE FOUND 2026-09-08 — fix in config, one lane run from lifting this rule.** A covered firefox window stops `requestAnimationFrame` (Windows occlusion tracking); Playwright's Windows-Firefox *stable* wait needs **5 consecutive rAF ticks**, so every click times out while `fill` (no stable wait) keeps working. Measured across 3 attempts per phase: **15 of 15 failing attempts had a dead rAF, 0 passing ones did** — and the stall is **sticky**, rAF does not restart when the window is uncovered, which is why one moment of occlusion poisoned a whole firefox session. `config/mcp-playwright-firefox.config.json` sets `widget.windows.window_occlusion_tracking.enabled=false`: covered rAF 121 vs 0, 6/6 clicks. **Restart the MCP server, then run one click-driven suite on firefox with the other lanes busy; this rule lifts when that suite is green** — evidence and procedure: `knowledge/automation/browser-quirks.md` §Firefox.
