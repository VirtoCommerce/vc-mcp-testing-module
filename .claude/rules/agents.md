...
>
> **Investigation 2026-09-08 (open):** Playwright's Windows-Firefox *stable* wait needs **5 consecutive rAF ticks** (1 elsewhere) while `fill` has no stable wait — that is why only clicks die, and the stall signature means **the refresh driver stopped ticking**. Measured run 3: window occlusion is NOT the cause (plain Firefox clicked fine fully covered, rAF 0), so the occlusion pref now in the firefox config is not the fix. A deterministic reproducer exists — `reducedMotion: 'reduce'` idles the page and stalls every click at *"visible, enabled and stable"* — and `firefox-click-probe` v4 tests candidate fixes against it. **This rule stays until that probe exits 0 on a machine that showed the bug** — table, mechanism and the flip procedure: `knowledge/automation/browser-quirks.md` §Firefox.
