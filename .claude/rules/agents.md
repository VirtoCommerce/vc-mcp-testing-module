...
>
> **Investigation 2026-09-08 (open):** Playwright's Windows-Firefox *stable* wait needs **5 identical rAF ticks in a row** (1 elsewhere) while `fill` has no stable wait — that is why only clicks die. Probe runs 1–2 were invalid (an off-viewport skip-link as target, and a cover window that landed on the other monitor of a dual-head desk), but they did rule out rect jitter — 1 distinct rect in 12 frames on every engine — and one accidental full cover reproduced the symptom exactly: rAF 0, stall at *"visible, enabled and stable"*. Occlusion is the candidate to prove; `config/mcp-playwright-firefox.config.json` already carries the pref. **This rule stays until `node scripts/maintenance/firefox-click-probe.mjs` exits 0 on a machine that showed the bug** — mechanism, run log and the flip procedure: `knowledge/automation/browser-quirks.md` §Firefox.
