...
**`qa-methodology/` — QA Methodology (13) — manual invocation:**
...
| `/qa-hotfix-check` | `VCST-XXXX [--envs=stable,regression] [--bundles=v14,v15] [--dry-run]` | Deliver an already-released hotfix onto the deployed `vcptcore-stable` + `vcptcore-regression` envs (bump `vc-deploy-dev` `backend/packages.json` → commit `VCST-XXXX: title` → "Cloud platform deployment" Action), wait for the deploy (Action green → `/api/platform/modules` version match), verify the fix live, comment on JIRA, then bump the latest-stable bundles (auto-detect + ASK). Delivery-only; gated writes, dry-run by default. Deterministic core: `scripts/hotfix-deliver.ts` (`npm run hotfix:deliver`). Third link after `/qa-bundle-check` → `/qa-hotfix`. | `SKILL.md` (env wiring + delivery/deploy-poll mechanics + verification + bundle-bump gate) |
