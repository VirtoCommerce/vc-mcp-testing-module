...
| `/qa-triage-results` | `[RUN_ID \| latest] [--fix] [--verify]` | No | Triage a completed regression run's FAILs: collect each failure + its trace/screenshots/console/network + failing CSV row → dedup + flag cross-run flaky → classify each (real-product-bug vs test-defect: bad steps / bad assertion / stale test data / stale test; vs flaky/env/known) → live-verify the real ones → auto-apply test-case fixes via `/qa-review-tests --fix` (confirm) + draft bug reports → STOP for human. Never files a tracker ticket, never triggers `/qa-fix`. Reuses `scripts/lib/regression-triage.ts` + `ci/agents/regression-triage-agent.md`. Also fixes the `history.json` flakiness feed. Backed by the `/qa-triage-results` skill. |
...
**`qa-methodology/` — QA Methodology (13) — manual invocation (except `/qa-evidence` + `/qa-sbtm`, which are auto-invocable reference-only skills):**
...
| `/qa-triage-results` | `[RUN_ID \| latest] [--fix] [--verify]` | Triage a completed regression run's FAILs: collect (trace/screenshots/console/network/CSV) → dedup + flaky → classify (real-bug vs bad-steps/bad-assertion/stale-data/stale-test vs flaky/env/known) → live-verify → route test-defects to `/qa-review-tests --fix` + draft bugs → STOP. Never files a tracker ticket. Deterministic core `scripts/lib/regression-triage.ts`; classifier `ci/agents/regression-triage-agent.md` | `SKILL.md` + `triage-taxonomy.md` (8 classes + worked examples) + `routing-and-fix.md` (per-class routing + write discipline) |
