...
| 2 | Test cases | `regression/suites/` (CSV) | Adding/updating test coverage. `/qa-test`-authored cases land here directly as `Automation_Status = Draft` (Step 3) and are flipped in-run to `Automated`/`Reviewed` at 5i after execution — never a run-scoped ticket CSV |
