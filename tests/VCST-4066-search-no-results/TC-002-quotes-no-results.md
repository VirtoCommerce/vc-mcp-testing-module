# Test Case: TC-002 - Quotes - No Results Display and Reset
...
|-------|-------|
| **Test Case ID** | TC-002 |
| **Test Case Name** | Quotes - No Results Display and Reset |
...
## Objective
Verify that when a user searches for quotes on the Quotes page and no results are found, the system displays a clear "no results" message with a "Reset search" button that restores the full list.

## Preconditions
1. User is logged into VirtoStart Demo Store
2. User has access to the Quotes page
3. User has at least one quote in the system
4. Test environment: https://vcst-qa-storefront.govirto.com

## Test Data
- **Valid search term**: [Existing quote number or product name]
- **Invalid search term**: "QUOTE999999NOTEXIST"
- **URL**: `/account/quotes`
...
|--------|--------|-----------------|
| 1 | Navigate to https://vcst-qa-storefront.govirto.com/account/quotes | Quotes page loads successfully |
| 2 | Verify that the search input field is visible | Search input field is displayed on the page |
| 3 | Verify that quotes are listed (if any exist) | Full list of quotes is displayed |
| 4 | Enter a valid quote number or product name in the search field | Search field accepts input |
| 5 | Observe the search results | Quotes matching the search term are displayed |
| 6 | Clear the search field and enter "QUOTE999999NOTEXIST" | Search field accepts the invalid search term |
...
| 11 | Verify page layout is consistent with other pages | Layout matches the design pattern |
| 12 | Click the "Reset search" button | Button responds to click action |
| 13 | Observe the page after reset | Full list of quotes is restored |
...
- **Design Consistency**: Matches Figma design specifications and other pages (Back in Stock, Orders, etc.)

### Functional Behavior
- Search executes for non-existent terms without errors
- No results page is non-blocking (user can interact with UI)
- Reset button clears the search and restores the full quote list
- Search field is cleared after reset
- Quote details remain accessible after reset
