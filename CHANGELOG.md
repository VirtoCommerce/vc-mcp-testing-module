...
- **Rotation** `scripts/test-cases/audit-queue.ts` (`tc:audit:queue`): risk tier → unresolvable-source last → oldest stamp → testCount, keyed by **file** not id. **Source axis** `scripts/test-cases/suite-source-map.ts` (`tc:audit:source`) resolves 113/120 suites and **never invents a repo name** — an unresolvable suite scores UNGROUNDED, because a wrong repo yields a confident `file:line` for unrelated code and manufactures a false CONFIRMED.
...
- **New suites:** storefront structured-data / SEO (`048d`, VCST-5536), saved-layout coverage (VCST-5367), sales-rep hub dashboard + admin/permission negatives + statistics GraphQL (`092`, `092b`, `093`, `050m`), notifications `057`/`058` reworked against VCST-5557 / PR #202, and a `catalog:read-only` RBAC fixture for PLAT-079. Manifest now carries **121 suites / ~3,985 cases / 38 selections**.
