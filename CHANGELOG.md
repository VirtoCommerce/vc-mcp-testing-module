...
- **Fix 1 — Stop-hook over-defers on non-owned background tasks** (`hooks/session-telemetry.mjs`). The phantom "1 pending subagent" was a `run_in_background` **SHELL** task belonging to a **different** session that the platform surfaced in `ev.background_tasks`. The checkpoint-vs-terminal check trusted `background_tasks.length` **exclusively**, but that array is not session-scoped, so a foreign task forced perpetual deferral — the terminal finalize never ran, `flagged[]` was never computed, no command span closed, and no clean/finding line surfaced. Now we defer **only** when a pending background item corresponds to one of **this session's own open AGENT ops** (matched by id/tool_use_id; a bg entry with no id falls back to kind and counts a genuine agent/subagent only, **never** a shell/bash task). Hard guard: **zero open agent ops ⇒ never defer** (finalize). The absent-field `freshOpenAgents` fallback + the STALL_MS orphan backstop are unchanged. When we do defer, `pendingSubagents` now reflects OUR pending agents, not global background noise.
- **Fix 2 — command span not attributed for the capture-enabling command** (`hooks/session-telemetry.mjs`). `/project-init` turns capture ON mid-run (its §0b consent), so its own `/project-init` UserPromptSubmit fired while capture was still a no-op → no command span opened, and the run's tool-level failures could never be flagged. Now, when the terminal finalize sees a `complete --skill "<name>"` marker for THIS session but no command/skill span of any kind, it **synthesizes** a command span (start = session start, end = now), rolls the session's orphaned blocking-error counts onto it, and closes it — so the run is represented by one real span and any failed tool op within it becomes flaggable. Guarded to that exact blind spot (a plain-dev turn emits no `complete`, and the sid-scoped marker guard blocks cross-session bleed).
- **Fix 3 — delivery scrubber withheld plugin internals as if client data** (`skills/vc-self-check/deliver.mjs`). `containsClientShape()` flagged ANY compound camelCase (`/[a-z][A-Z]/`) as client-shaped, so the plugin's OWN symbols (`pendingSubagents`, `sawPluginSpan`, `anySkillSeen`, `openOps`, `roleStates`, `currentCommand`, `freshOpenAgents`, …) were withheld and every finding row was downgraded to "(client-specific evidence withheld)". Now: (a) a **plugin-symbol allowlist** is derived at delivery time by scanning the plugin's own source (`hooks/*.mjs`, `skills/**`, `commands/**`, `ci/lib/**`, bounded to `pluginRoot()`, cached) for declared identifiers and unioned with `SAFE_TERMS`; (b) the camelCase heuristic is narrowed — a **lowercase-first** camelCase token is a code identifier and is NOT flagged on shape; compound-flagging is reserved for **PascalCase / Capital-first** tokens (`AcmeCorp`, `CartController`) and dotted namespaces. **Client-data protection is unchanged:** the hard gates — configured client terms from `project-profile.json` (any case, incl. inside underscore/dot/hyphen-joined identifiers), path/URL/email/ticket/UNC/drive shapes, and the shared `redact()` secret scrub (JWT/PAT/AccountKey/SAS/Basic-auth/PAN) — all stay fully intact and are applied first.
- Tests: `scripts/unit/session-telemetry.test.mjs` gains a Fix-1 defer/terminal ownership matrix + the Fix-2 synthesized-flagged-span case (and the two tests that encoded the old buggy behavior — `terminal (foreign background_task)…` and `opt-in mid-run…` — were updated to the corrected semantics). New `scripts/unit/deliver-scrub.test.mjs` covers the Fix-3 acceptance cells (plugin internals survive; configured client org / client path / PascalCase class / embedded secret still blocked). 235/235 unit tests green.
