...
- **Code-quality follow-ups (PR #143 review, Lenajava1):** extracted `cmdFinalize`'s deeply-nested `suppressReason` ternary into a pure `computeSuppressReason()` with early returns (behavior-identical, still audit-only); and lifted the client-repo push→severity mapping into a pure, unit-tested `clientRepoWriteSeverity()` in `probe-lib.mjs`, now shared by both the GitHub and Azure client-repo paths in `verify-access.mjs`.
- **Client-push severity is now mode-aware.** A missing client-repo push is **FAIL** on a **fresh onboarding** (must not finish onboarding with a token that can't push) but only **WARN** on a **day-2 re-verify** (`/project-init --check` / `--add-env`, which pass `verify-access.mjs --existing`) — an already-onboarded project whose client token later regressed to read-only is flagged, not hard-blocked on a routine re-check. Default is strict (FAIL), so a forgotten `--existing` over-blocks (safe) rather than under-blocks. Addresses the review's rollout-risk note.
