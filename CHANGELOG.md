...
- **D2 — always ignored.** `.mcp.json`, `.claude/settings.local.json`, `.env.local`, `.env.*.local`, `project-profile.json` and `.vc-fix/` are written as a labelled `.gitignore` block **before** any of those files are created, so `.mcp.json` never exists un-ignored even briefly. A project with no `.gitignore` gets one. The two generated entries are derived from the **resolved destinations**, so `--out`/`--settings` cannot route a credential-bearing file past the block; a destination outside the project root warns loudly instead.
...
- **B4 — a regression guard on the ARTIFACT.** `verify-access.mjs` adds two readiness rows auditing `.mcp.json` **and** `.claude/settings.local.json` — guarding only the first would leave the value's new home unchecked. The check is structural (a credential-shaped key whose value is neither a `${VAR}` ref nor an unresolved `<PLACEHOLDER>`) plus the known-prefix matcher now **shared with `hooks/redact.mjs`** (the audit's own shorter copy let `glpat-`/`xoxb-`/`sk_live_`/`AKIA`/JWT through), and it walks the whole server def — `headers`, `env`, `args[]`, `url`, nested bags — because the producer had learned to substitute into `args[]` while the auditor still read two bags. Grading is pure and table-tested, by **actual exposure**: FAIL only when a credential is present in a file git would commit — including one already **tracked**, which `git check-ignore` reports as not-ignored and which a `.gitignore` rule cannot fix (that row says `git rm --cached`). Outside a git repo, ignore-state is never a finding. Key paths only ever reach the table; a value never does.
...
Deliberately out of scope: the `.claude/` project surface carries an older generation of this generator with the same defects. It is not under the `mirror-parity` gate and this repo's own committed `.gitignore` already covers `.mcp.json`, so the D2 exposure does not exist there.
...
`knowledge/oracles/e-commerce-edge-cases-library.md` had **no declared write owner and no gate of any kind** — test cases cite its sections in `Edge_Case_Refs` and nothing ever checked those citations resolved. The first run found **20 dangling ECL ids cited by ~65 live cases across 7 suites**, including `ECL-13.4`, which has never existed.

- **New gate `scripts/knowledge/lint-ecl.ts` (`ecl:lint` / `ecl:audit:collect`)** — the ECL twin of `bl:lint`: **ECLC-001** dangling citation · **ECLC-002** uncited section · **ECLC-003** unparsable suite (surfaced, never read as clean — `bl:lint`'s BLC-005 rule) · **ECLL-001** duplicate section · **ECLL-002/003** Appendix D ↔ body coherence · **ECLL-004** zero-padded spelling. `--filter` narrows the *report*, never the *scan*.
- **Library audit.** Most dangling clusters were the library **missing content the authors expected**, not authors mis-citing — so the sections were added at the exact numbers already cited (§1.4–1.8 layout, §6.4 file import, §7.4 mobile/responsive, §10.3 Admin SPA blade availability, §14.10 admin order edit, and a new **chapter 15** for accessibility). Two duplicate rows merged; drifted `BL-SEARCH-*`/`BL-ORG-*` refs (domains that don't exist) corrected. Suite remap took `ecl:lint` from 20 High → **0**.
- **The two oracle axes are now one skill.** `/qa-review-oracles [bl|ecl|all] <scope>` — they already shared their entire mechanism (sole writer `ba-system-analyzer`, three triangulation axes, the 3-source evidence bar, auto-apply-vs-proposals split, stable-ID contract, deterministic lint core); only the entry *shape* differs, and that lives in `bl-audit-criteria.md` / `ecl-audit-criteria.md`. **`/qa-review-bl` is retained as an alias**, not renamed — `/qa-test-lifecycle` Phase 4c auto-runs it and a silent break there would be invisible.
- **The gate proves a ref EXISTS; it cannot prove the ref is RIGHT.** Nine loyalty cases cited `ECL-13.2` ("Subscription & Recurring Billing") meaning `ECL-13.3` ("Loyalty & Points") and no gate could object — that semantic call is `/qa-review-tests` Dimension 6, the same split as GRD-001 ↔ Dimension 11.

### Added — `/qa-perf-measure`: deployed-env performance measurement (VCST-5637) (#219)

Turns the VCST-5637 measurement run into a repeatable skill. The naive version of that method was wrong **four separate ways**, each producing a plausible number that was published and then withdrawn — so every lesson ships as an enforced rule.

- **Skill + command + recipe:** `.claude/skills/qa-perf-measure/`, `.claude/commands/qa-perf-measure.md`, and `.claude/knowledge/execution/es-call-ab-method.md` (the KQL, gotchas, fixture filter, confounds and worked examples — previously **untracked and referenced by no index**). Generalized past the originating ticket's ES/cart specifics: any request-serving surface (GraphQL xAPI, REST, Admin API), any dependency type (search, SQL, cache, outbound HTTP).
- **Counts transfer across environments; latency does not.** Backend work per request is counted by type via an App Insights `requests ⋈ dependencies` join on `operation_Id`; the residual reads N+1 → batch, flat >1 → collapse distinct shapes, flat =1 → floor.
- **Rules from the four wrong answers:** an **instrument routing table** (L1 BenchmarkDotNet mocks the very I/O under test; L3 `dotnet-trace` can't attach to a remote deployment) · **paired positive + negative controls in the same window**, else the delta isn't published · deploy boundary in **UTC with a guard band** (a `merged_at` `Z` read as a local offset once shifted a cut 3 h) · **verify the fixture exercises the path** (`addItem` no-ops silently on zero stock and `minQuantity > 1`) · **warm vs warm**, discarding each cell's first request (1613 ms/75 SQL cold vs 20 ms/3 SQL warm on the *same* build) · discover, then measure *different* targets · controlled A/B over observational traffic, `totalDeps` always carried as control.
- Measure-and-report only — never files a tracker item, transitions one, or opens a PR.

### Changed — `/qa-test` is a tiered, gate-verified lifecycle (#206, #208, #213)

The command grew from a linear script into a routed pipeline whose effort tracks risk, with independent verification only where it pays.

- **Two-axis routing at Step 1a**, single source of truth `.claude/knowledge/execution/ticket-routing.md`: first the **flow** by ticket type × status (a fix-ready Bug is a *verification* → `/qa-verify-fix` RED→GREEN **inline**; a hotfix-status Bug → `/qa-hotfix-check`; a Sub-task inherits its parent; else `feature-test`), then within `feature-test` a **FAST** path (P2–P3, single-layer, single-domain: skip the 1c/1d BA agents, one execution agent, inline self-checks) or a **FULL** path (feature/Story, P0–P1, cross-layer, ≥2 domains, critical-revenue or unclear surface). *When in doubt → FULL.*
- **Independent verification only at the three hard-STOP gates (Step 3, 5b, 5g), FULL path only** — a **fresh `qa-lead-orchestrator` in §Verifier Mode**, never the inline orchestrator and never the step's own doer: it re-derives evidence from source (`suites:review` / `td:validate` / `compute-metrics.ts --gate feature`, a live re-check on a **different browser lane**) and returns APPROVE/REJECT. REJECT → reason + fix → **re-verify once, then STOP**.
- **One test-case pipeline, one promoter.** Step 3 reuses the same skills `/qa-test-lifecycle` Phases 3–4 run and appends new cases **directly into `regression/suites/`** as `Automation_Status = Draft` (the runner does not skip Draft, so Step 4's scoped regression executes them); 5g flips the eligible ones `Draft → Automated`/`Reviewed` in place — last and non-blocking — reverting non-promotable rows. `/qa-test-lifecycle` Phase 6P remains the promoter for handoff / re-promotion / non-`/qa-test` sources.
- **Step 5a reuses `/qa-triage-results <RUN_ID> --fix`** instead of re-deriving the failure taxonomy ad hoc, and the Step 5 close-out was reordered (comment before status transition; promotion last).
- **`--iterate` (default `--max-rounds 2`)** turns the close-out into a bounded test → fix → re-test loop: a FAIL auto-runs `/qa-fix` (G0–G7, never merges) → `/qa-deploy-pr` deploys the **prerelease** (confirm each deploy — the loop always re-tests an *unmerged* build, so the never-auto-merge guard is never touched) → re-runs the failed cases + change-scoped regression, then STOP for a human.
- **Epic-aware:** Step 1a resolves a story's parent Epic and classifies siblings (Done = integration surface, In-progress = dependency/possible BLOCKED); **`--epic <KEY>`** tests child stories in series with state carried story→story and rolls the per-story gates into an Epic-level GO/NO-GO.
- **Step artifacts are terminal-only.** `ac-analysis.md` / `testing-checklist.md` / `test-execution-report.md` had no reader beyond the run that wrote them; only `summary.json` + screenshots persist. Report policy updated to match.

### Fixed — regression case IDs are globally unique; a new collision now hard-fails (#207)

**223 → 0.** 223 case IDs appeared in more than one suite CSV. Not cosmetic: the runner keys per-case results and failure evidence by **bare case ID** (`suite-*-results.json` rows, `traces/{TC-ID}-FAIL-trace.json`, `regression-triage.ts` fingerprints), so two suites both declaring `CAT-001` let one run's evidence silently overwrite the other's — a real failure can read as someone else's pass.

- **Two collision kinds, two conventions.** *Re-prefix* where different layers shared a prefix — storefront keeps the bare prefix, admin takes an `…A` suffix (`CATA-*`, `ORDA-*`, `SRCHA-*`); where one prefix meant two unrelated things the documented owner keeps it (067 keeps `WL-*`, so wishlist 050h became `WISH-*`; 050i keeps `CFG-GQL-*`, so the interlopers in 072/072c became `CFG-XAPI-*`), and the more specific suite is qualified (`CPN-SMK-*`, `SR-EMB-*`). Applied to the **whole prefix in each file**, so a file keeps one coherent namespace and cannot collide again. *Renumber into a free range* where both suites legitimately share one domain (035, 032, 003, `CFG-TEXT`/`CFG-VAR`).
- **309 row-ID rewrites + 40 in-suite cross-reference rewrites across 15 suites**, verified mechanically rather than by eye: reverse-applying the rename map reproduces HEAD byte-for-byte, so only ID tokens changed and every CRLF / bare CR / escaped quote is untouched. `suites:review` finding tallies identical before/after.
- **Enforced going forward** by `npm run suites:lint` (`findDuplicateCaseIds`, unit tests in `scripts/unit/suite-global-case-ids.test.ts`) — it scans every CSV on disk, orphans included, and **hard-fails**; unlike `CSV_LINT_BASELINE` there is no burn-down set, because the corpus is at zero.

### Added — stale-test-case audit: `/qa-review-tests` Dimension 11 + a scheduled one-suite-per-day twin

`lint-test-cases.ts` GRD-001 verifies an assertion **carries** a grounded provenance tag; it never verifies the tag is **true**. A `{DOC}` whose doc changed, an `{OBSERVED}` from a six-month-old build, a `{BL}` citing a retired invariant all lint green. `/qa-review-bl` closed this for the BL oracle; this ports the mechanism to test cases.

- **Dimension 11 — Behavioral Triangulation (`--triangulate`)** triangulates each *assertion* against docs (VirtoOZ) + live (playwright) + source (GitHub MCP) → CONFIRMED / DRIFT / MISSING / CONTRADICTORY / UNGROUNDED / RETIRE (TRI-001…006). Auto-applies only **CONFIRMED** (refresh the `Audited:` stamp) and **DRIFT** (rewrite only the drifted assertion); the other four are proposals, because deprecation silently removes coverage and authoring needs review. Deploy lag (source has the fix, live does not) is CONTRADICTORY, never DRIFT. Judgment rules split into `triangulation-criteria.md`.
- **No CSV schema change** — the stamp appends to the free-text `References` column alongside the existing `Synced:`/`Corrected:` stamps, so the 15-column contract is untouched, and **the stamp IS the rotation state**: no ledger file to desync, and a skipped day leaves that suite at the head of the queue.
- **Scheduled twin** `ci/run-suite-audit.ts` (`npm run ci:audit`) + `.github/workflows/suite-audit.yml` audits **one suite per weekday** and lands each as its own **draft PR** — that PR is the human gate replacing `--fix`'s confirm prompt; never auto-merges. `--ci` holds a *narrower* privilege than interactive `--fix` (only what the linter proved or three agreeing axes confirmed), and a post-apply gate reverts any edit that introduces a new Blocker/Critical. CI has one browser lane, so the prompt overrides the SKILL's 3-agent fan-out.
- **Rotation** `scripts/test-cases/audit-queue.ts` (`tc:audit:queue`): risk tier → unresolvable-source last → oldest stamp → testCount, keyed by **file** not id. **Source axis** `scripts/test-cases/suite-source-map.ts` (`tc:audit:source`) resolves 115/121 suites and **never invents a repo name** — an unresolvable suite scores UNGROUNDED, because a wrong repo yields a confident `file:line` for unrelated code and manufactures a false CONFIRMED.
- Also fixed: `ci:audit` no longer hard-requires `ANTHROPIC_API_KEY` for its deterministic (non-agent) phases.

### Added — `vc-perf`: a second marketplace plugin — the three-layer performance loop (#136)

`plugins/vc-perf/` (`0.2.6`, depends on `vc-fix >= 0.7.0`, listed in `marketplace.json`): **L1** BenchmarkDotNet A/B over x-module targets, **L2** a k6 load harness against the Aspire-hosted backend with `dotnet-counters`, **L3** `dotnet-trace` allocation/CPU/DB attribution via perftools — plus a `perf-analyst` agent that ranks optimization candidates from the three layers' artifacts and a `perf-loop` orchestrator. Advisory only, never a CI gate; it reuses `vc-fix`'s onboarding, routing, and backend dev/review agents rather than duplicating them.

### Fixed — self-diagnostics: capture everything, judge later; artifact-free delivery (VCST-5582, PR #172)

The collector's non-success test at span close was simultaneously the **retention**, **analysis-scope** and **surfacing** decision, so any signal the deterministic classifier didn't recognise **ceased to exist**. A real `/project-init` run printed a WARN in its own readiness table (the Azure Bug field contract was never scanned: HTTP 400 → stderr → exit 0) and then self-diagnosed `no plugin issues detected` with `flagged:[]`. Capture, judgement and surfacing are now three separate layers.

- **Capture is forbidden from deciding relevance.** Every anomaly is a durable `type:"obs"` record with **no severity/verdict field** — closed `class` vocabulary, slugified `subject`, taxonomy `code`, aggregated by signature (caps announce themselves as `capture_truncated`, never truncate silently). New sources: the transcript's `toolUseResult` sidecar (**stderr on an exit-0 tool**, `interrupted`, `Exit code N`), self-labelled degradation prose in stdout *and* stderr, every non-PASS row of `verify-access.mjs` and every failed scan in `discover-tracker.mjs` (via the collector's new `obs` subcommand), the written profile's shape, signals on spans that end `success`/`recovered`, sub-agent sidechain errors, and the collector's own health.
- **Two invariants make the original failure unrepresentable:** `verdict:"clean"` requires `observations.total === 0 && flaggedTotal === 0 && !scanErrors`, and any `self_reported_warn`/`_fail` forces at least `attention`. `decision.verdict` now describes the **run** and is derived from counts alone; `surfaceDecision`/`suppressReason` describe the **UI choice** — conflating them is how a `failed` span once landed in a record calling itself `clean`.
- **Surfacing is bounded by a HARD routing class set** (`self_reported_fail` · `degraded_artifact` · `script_exit_nonzero` from a **plugin-owned** script only — anchored to the installed plugin root, so a client's own failing `node ./scripts/build.mjs` is recorded but never arms the diagnostician) plus per-signature dedup, with a **grown occurrence count** re-qualifying a recurrence. `self_reported_warn` / `http_non2xx` / `collector_contention` were **demoted out of routing** (the old set fired on essentially any new signal, costing the operator a whole extra turn on S2/S3 friction) but still forbid the word "clean". A turn now emits **one info line and at most one question** — the 3-option cleanup prompt was removed, since the 24 h age-cap already reclaims leftovers.
- **Delivery is artifact-free and struct-driven.** `/vc-self-check` runs via the `self-check-diagnostician` subagent and returns a **validated finding struct**; the local `DIAG-*.md`/`.json` and the whole `--diag` → markdown-table → sidecar-JSON source-precedence chain are **gone** (parsing a human-written `.md` for a closed vocabulary was the shared root cause of the header-backtick and `skill: other` defects — fixed by deletion). `deliver` reads the struct via `--input <file>`, files **one GitHub Issue per finding**, and routes by token rights through **two routes only, `issue` / `local`**: the former `pr`/`fork-pr` hand-offs printed commands and sent nothing, so the *more* rights a token had the *less* was delivered. A transient probe failure retries once before falling back to `local`.
- **Schema v2** added `subject` (a closed enum naming *which* operation misbehaved — `ado_create_workitem`, `browser_login`, …) and `blockedDeliverable`, because a v1 row read `other | BROKEN | S1 | …` and couldn't distinguish an Azure-Boards field-contract blocker from a credential-handoff gap. `subject` is a **mapping, never an echo**. A containment gap in `proposedFix` (IP + work-item state name) was closed, and an **anti-confabulation gate** grounds vendor error identity against session telemetry.
- **Consent + dedup:** delivery consent must use the **`AskUserQuestion` tool**, never a prose yes/no; dedup is **OPEN-issue only**, so a closed prior issue no longer swallows a recurrence. Deletion now requires the *analyze* step — a session holding an un-diagnosed finding survives the 24 h age-cap and `purge-inactive`.
- **Onboarding + client hardening:** 7 `/project-init` defects found on a live client deployment (step-header framing, a question the operator can't scroll past, both self-diagnostics consents asked together, the `GITHUB_FIX_BUGS_TOKEN` `where:` navigation path), a hardened ADO client that loads `.env.local` before ADO auth, no more false stalls on consent questions, a pinned `playwright-mcp`, and a closed telemetry exit-code blind spot. A `Bash` echo of plugin source no longer mints a false `self_reported_fallback`.
- **`.claude/` is deliberately NOT registered as a second collector.** `vc-fix@vc-tools` is enabled at the user level and ships its own `hooks.json`, so registering the mirror too ran **two** collector processes per event against the same files — duplicated spans and racing `saveState()` writes (observed: two `finalize` records 12 ms apart reporting 292 vs 290 spans). The mirror file stays (byte-identity is CI-enforced); `collector_contention` detection remains as a backstop.
- Plugin bumped **0.8.2 → 0.8.3**.

### Removed — the retired release suite `080`, and a lint that hard-fails on a missing suite CSV

The master release suite `080` (`_release/080-full-regression-release.csv`) had its CSV deleted while the manifest entry and the `release` selection survived, so `/qa-regression release` **resolved to a missing file — running zero cases while reporting a valid selection**. The manifest entry and the selection are removed; `npm run suites:lint` now hard-fails on any declared-but-absent suite CSV so it cannot recur silently. For a major release use `full` or a plan-driven `sprint` selection.

### Changed — report policy: retention + prune tooling, and BA domain folders

- **New §9 Retention.** Ephemeral run folders (`reports/regression/{REG-*,SMOKE-*}`, `TLC-*`, `COV-*`, `MONITOR-*`) age out locally via `npm run reports:prune -- --apply` (30 days, run date parsed from the folder name); git-tracked `reports/tickets/<Sprint>/<TICKET>/` age out via `--target=tracked --apply` (`git rm` + one commit, gated by the last **commit** date, current sprint always skipped). Durable categories never prune. Tool: `scripts/maintenance/prune-old-artifacts.mjs`, dry-run by default, `--target=tracked` must be named explicitly. First run pruned `vc/shared/archive/sprints/` Sprint26-02 → Sprint26-12.
- **BA reports gained domain subfolders** (`reports/ba/<Domain>/`) and the category list was restated as the **ten** allowed categories, with the `/qa-test` terminal-only carve-out and the `/qa-verify-fix` triple (`verification-report.md` + `verification-summary.json` + `evidence.html`) written down — including the rule that the committed `evidence.html` **references** its sibling screenshots rather than inlining them as base64 (one inlined PNG cost 124 KB on a single 124 000-character line).

### Fixed — test-data, seeding, and deploy tooling

- **Seeding refuses an undeclared `ENV_RISK`** rather than guessing, and the silent per-env password fallback is now surfaced instead of resolving quietly to the wrong secret.
- **Teardown no longer orphans an address** when its CSV row has already been deleted, and `aliases.<env>.json` write-back terminates with a newline.
- **`ADDRESSES_PER_PAGE` is gated against the storefront** — the previous constant produced a false PASS. Suite 011 pagination coverage was unblocked by 22 seeded TechFlow org addresses, and the phantom `techflow-org-addresses` JSON was purged from every live reference.
- **`/qa-deploy-pr`** keeps the manifest edit minimal for empty-`AzureBlob` and re-pin cases, and **asserts the intended end state before committing**.
- **The `post_edit` typecheck gate widened** from `scripts/lib/*.ts` to **all of `scripts/**/*.ts`** — `scripts/deploy/`, `scripts/unit/`, `scripts/hotfix/` were silently unchecked, so a green run said nothing about them.

### Added — regression coverage, oracles, and fixtures

- **New suites:** storefront structured-data / SEO (`048d`, VCST-5536), saved-layout coverage (VCST-5367), sales-rep hub dashboard + admin/permission negatives + statistics GraphQL (`092`, `092b`, `093`, `050m`), notifications `057`/`058` reworked against VCST-5557 / PR #202, and a `catalog:read-only` RBAC fixture for PLAT-079. Manifest now carries **121 suites / ~3,985 cases / 37 selections**.
- **Behavioral triangulation audits** landed for suite `015` (the first real one), `011`, and `028` Cart Core (51/52 cases).
- **Oracles:** `business-logic.md` Domains 1–19 deduped and `BL-SR`/`BL-SREP` merged into one Domain 20; five notification invariants promoted with five more drafted from the trigger audit; VCST-5618 401/403 auth fix covered by a new BL invariant + regression cases.

### Fixed — PR #143 review round 4 (9-agent review) — 3 correctness fixes + mirror guard

A fresh 9-agent review confirmed containment/consent SOUND (no upstream client-byte channel, opt-in default-off, mirror byte-identical, incremental transcript scan correct) and surfaced three MED correctness defects in the auto-report path — all fixed here, each with a regression test. Full suite 392/392 green.

- **#1 — `/project-init` self-diagnosed `failed` on a recovered probe (spurious upstream issue in `auto`).** The synthesized command span (the capture-enabled-mid-session blind spot, Fix 2) rolled up the **cumulative** `state.totals.tool_error/permission_denied/hook_failure`, which count every errored `tool_result` — even one a later retry self-corrected. A probe-heavy `/project-init` (a flaky network probe, a Bash exiting non-zero then re-run, a NOT-READY exit-1) therefore classified BROKEN and, in `feedback.mode=auto`, filed a bogus upstream issue. The synth span now **adopts the session's orphaned (parentless) ops** — buffered in `state.orphanOps`/`orphanDetails` + a session-level `untiedFailure` flag — so `classify()`/`allErrorsRecovered` apply the SAME self-correction test a real span gets: a retried-then-recovered probe is `recovered` (S3, unflagged), a genuinely-unresolved failure stays `failed`. Regression test: a recovered orphan probe → `recovered`/unflagged (`session-telemetry.test.mjs`); the genuine-failure test is unchanged and still green. _(A re-review of this fix caught NA-1: an UNTIED blocking failure — a top-level `tsc`/hook echo with no paired op — has no orphan op for the synth to derive a signal from, so deriving signals ONLY from adopted ops silently under-flagged it (the pre-fix code caught it via `state.totals`). Fixed by a per-class `state.untiedSignals` counter folded into the synth's signals, so an untied failure still forces `failed` — the established asymmetry a real span gets — while recovered TIED probes (orphan ops, subject to `allErrorsRecovered`) stay `recovered`. Regression test added for the pure-untied case.)_
- **#2 — `silent_suspect` vs `failed` forked the dedup fingerprint.** The primary jsonl path emits the Tier-1 outcome verbatim (a silent span → `silent_suspect`) while the DIAG-fallback derives it from the verdict (BROKEN → `failed`); since `findingStructSig` included `outcome`, the SAME defect fingerprinted differently across a jsonl-present vs DIAG-only client → two upstream issues instead of "+1 occurrence", specifically on the highest-value silent-failure class. `findingStructSig` now canonicalizes `silent_suspect`↔`failed` to one signature token (the emitted struct/body still carries the true outcome). Regression test asserts the two fingerprints converge and that a genuinely different verdict stays distinct. _(Migration note: this changes the fingerprint hash of an existing `silent_suspect` finding, so a dedup vs an issue ALREADY filed upstream under the old hash may re-file ONE duplicate, then self-heals. Bounded + acceptable — the subsystem is opt-in + consent-gated, only `feedback.mode=auto` files unattended, and it is still feature-branch, so the old-scheme upstream population is ~zero; the change deliberately makes jsonl-path and DIAG-fallback clients converge instead of forking every session.)_
- **#3 — a transcript rotation/compaction re-emitted spans → duplicate findings + forked fingerprint.** `scanTranscript`'s `size < scannedBytes` branch re-scans from scratch and re-emits a span with a FRESH id, which `reduce()`'s id-dedup can't catch → duplicate rows and a forked fingerprint (its sig is the sorted finding list). `reduce()` now collapses findings sharing a structural signature (keeping the first; cross-SESSION counting stays `mergeStructs`' job). Regression tests: same-signature re-emitted spans collapse to one and don't fork the fingerprint; different-signature spans are both kept.
- **Defense-in-depth + guards (from the review's Low/Suggestion notes).** (a) The dedup fingerprint now also appears as a **visible** body line, not only in an HTML comment — GitHub issue Search does not reliably index `<!-- … -->`, which made `findDuplicateIssue`'s Search branch always miss and silently degrade to the first-100-issues fallback (the DED1 failure it was meant to fix). (b) `session_start.cwd` is run through `redact()` (local-at-rest hygiene — it never reaches the upstream struct regardless). (c) `isAllowedUpstreamRepo` rejects `..`/leading-dot path traversal in the repo segment. (d) New `scripts/unit/mirror-parity.test.mjs` **fails CI on any byte-drift** of the four self-diagnostics files between `plugins/vc-fix/` and `.claude/`, so the containment hardening can't silently ship on only one surface. (e) `redact.test.mjs` gains direct `apiKey`/`X-Api-Key:` coverage; the implicit-`any` at `qa-fix-routing/repo-router.ts:218` is typed.
- **Deliberately deferred (unchanged, tracked).** S4 (`cmdFinalize` helper extraction — a pure refactor of a hot, heavily-branched, well-tested path; no correctness impact) and the ADO write-probe `401-before-400` premise (M1 — needs a purpose-made read-only PAT to verify live; the `verify-ado-write-probe.mjs` harness is present; flipping `400→WARN` would regress the happy-path onboarding for everyone, so the classification is left as-is).
...
- **Wiring / cleanup:** wired `SubagentStop→agentstop` into `.claude/settings.json` (sub-agent skill spans are now captured at SubagentStop, not only via the terminal Stop delta) and removed the dead `.claude/hooks/hooks.json` (a project-scoped surface is wired via `settings.json`; that stray manifest used `${CLAUDE_PLUGIN_ROOT}` and was never loaded — the plugin's own live `hooks/hooks.json` is untouched).
- **Not done (deliberate / tracked follow-up):** `.claude` reconcile `feedback`-consent + dotted-`--set` (`setDeep`) modernization — omitted rather than half-shipped; it fails safe (absent `feedback` ⇒ deliver defaults to `ask`, still `--confirm`-gated) and the distributed plugin covers it. `probeGithubUpstream`/`resolveGithubToken` unit coverage (needs a `fetchImpl` seam).
...
- **F1 (dead scrubber vs docs).** The free-text client-shape scrubbers `scrubText`/`isClientSpecific`/`containsClientShape` (and their helpers `clientTerms`/`SAFE_TERMS`/`pluginSymbols`/`PLUGIN_FILE_RE`/`REDACTED_PLACEHOLDER_RE`) plus the dead `readSessionFeedback` were **removed** from `deliver.mjs`: once the upstream artifact became enum-only (the closed schema, above) they had **zero call sites** in the send path, yet CLAUDE.md + the ADR still advertised them as an active "defense-in-depth" backstop. Deleted the code, corrected the docs to state the truth (the closed schema `validateUpstream` is the sole upstream guard; `redact()` remains on the collector's local persist path). Removed `deliver-scrub.test.mjs` (it tested only the deleted code) and repointed `test:containment` at `upstream-reduce` + `deliver-feedback`; the shared `redact.mjs` secret shapes stay covered by `session-telemetry.test.mjs`.
- **F2 (`.claude/` mirror still carried the leak class).** Resolved by **syncing** rather than documenting an exception: `hooks/redact.mjs`, `hooks/session-telemetry.mjs`, `skills/vc-self-check/deliver.mjs`, and `skills/vc-self-check/upstream-reduce.mjs` are now **byte-identical** across `plugins/vc-fix/` and `.claude/`, so the hardened secret redaction + the default-deny closed schema ship on BOTH surfaces. CLAUDE.md + the ADR updated to reflect the synced state (the mirror may still lag only on non-self-diagnostics prose).
- **F3 (DIAG-fallback skill → "other").** `parseDiag` now normalizes the DIAG Skill cell (`/qa-fix (command)` / `/qa-bug (skill)`) to the bare enum name before the jsonl-purged fallback reduces it, so per-skill fidelity is no longer silently lost; unknown names still fall through to `"other"` (fail-safe preserved). Regression test added.
- **S2 (untested security-relevant helpers).** New `scripts/unit/deliver-route.test.mjs` covers `isAllowedUpstreamRepo` (the `VirtoCommerce/*` misroute guard), `resolveRoute` (the pr/fork-pr/issue/local matrix), `findDuplicateIssue` (dedup match, `fetch` stubbed), and `purgeSession` (exported; the "delete ONLY this session, never others" guarantee + the short-sid guard).
- **S3 (`scanTranscript` ~O(n²)).** The collector now reads only the transcript bytes appended since the last scan (a byte offset kept at a line boundary), instead of re-reading + re-splitting the whole transcript on every skill-boundary/Stop. Guards: a shorter-than-offset file (rotation/truncation) or a pre-S3 `.state.json` (mid-upgrade) falls back to a one-time full read honoring the legacy line cursor; a partial trailing line (and any torn multibyte char at EOF) is never consumed until completed. Regression tests for the partial-line and rotation cases added.
- **S6 (test boilerplate).** Extracted `scripts/unit/_test-helpers.mjs` (`withTempDir`/`withTempHome`) and adopted it in the new route/purge tests.
- **Not done (deliberate):** S4 (extract `cmdFinalize`'s decision helpers) — a pure refactor of a hot, heavily-branched path; deferred rather than risk a silent behavior change in this PR. NA#4 (split the PR) — the security fix is already closed + tested on this branch; splitting now costs more than it saves.

### Changed — self-diagnostics upstream contribution: default-deny CLOSED SCHEMA (ends the client-data-leak class)

The `/vc-self-check deliver` upstream path leaked client data to the PUBLIC `VirtoCommerce/vc-mcp-testing-module` repo three review rounds in a row (bare `github_pat_` tokens, lowercase camelCase client identifiers, all-caps slugs, …). Root cause was architectural: the outbound report was composed from **free-form text derived from the client session** (LLM-authored DIAG cells `signal`/`rootcause`/`fix` + `/vc-feedback` prose) and defended by a **denylist** (`redact.mjs` + `deliver`'s `containsClientShape`/`scrubText`). A denylist over an unbounded input space always trails reality.

**The trust direction is inverted: the upstream artifact is now built ONLY from a fixed, plugin-authored vocabulary of enum/number primitives — a closed schema enforced by TYPE, so there is structurally nothing to leak.**

- **New `skills/vc-self-check/upstream-reduce.mjs`** — the pure, deterministic reducer: `reduce(local) -> UpstreamSignal` (a struct whose every field is a closed-vocabulary enum or number: skill / verdict / severity / outcome / signalClass / struggle[] / **errorCode** taxonomy / toolFamily / repo**Kind** / retries / occurrences + feedback **counts**; ZERO free-text fields), `validateUpstream(struct)` (runtime boundary barrier — coerces any out-of-vocabulary value to a safe default: `UNKNOWN`/`other`/`none`, clamps numbers), `classifyError(snippet)->code` (LOCAL raw-text→closed-taxonomy classifier — only the CODE travels, never the text), and `fingerprintStruct` (hashes the STRUCTURAL enum tuple, never raw text). Spec: `knowledge/diagnostics/upstream-schema.md`; rationale: `knowledge/diagnostics/adr-upstream-default-deny.md`.
- **`reduce` reads ONLY the structured collector jsonl** (span records + feedback verdicts). The LLM-authored DIAG free-text cells NEVER enter the upstream path; `/vc-feedback` prose is dropped (only 👍/👎 counts travel); repo/module/org NAMES are never sent (only `repoKind`). `verdict`/`severity` are derived deterministically from the Tier-1 `outcome`, not LLM judgment. Fallback when the jsonl was purged: an **enum-only** struct from `parseDiag`'s validated verdict/sev/skill, dropping all free-text — still type-safe.
- **`deliver.mjs` rewritten** to build the outbound title/body SOLELY from `validateUpstream(reduce(...))`; the old text-derived `fingerprint`/`findingSig` were removed (superseded by `fingerprintStruct`). `redact()` / `scrubText` / `containsClientShape` **stay as local defense-in-depth** but the upstream path no longer DEPENDS on them. _(Superseded by round-2 F1 above: `scrubText`/`containsClientShape` were subsequently removed as dead code once the upstream artifact became enum-only; only `redact()` remains, on the collector's local persist path.)_ The collector `hooks/session-telemetry.mjs` is **unchanged** (capture stays opt-in, never throws). Consent (`feedback.mode`) + §2a route resolution unchanged.
- **Tests:** new `scripts/unit/upstream-reduce.test.mjs` — a property/fuzz proof that adversarial client-shaped strings (secrets of every format, org names, camelCase/PascalCase/all-caps/all-lowercase identifiers, Win/UNC/POSIX/extensionless paths, URLs, emails, tickets, non-latin) injected into every LOCAL slot appear in NONE of the serialized struct, and every field ∈ its vocabulary (leak impossibility by construction) + classifier/validator/fingerprint/mapping units. `scripts/unit/deliver-feedback.test.mjs` moved to the struct-based `buildDraft` contract + `readSessionRecords`; `deliver-scrub.test.mjs` unchanged (the local scrubbers it covers still exist). End-to-end replay (single + `--batch` + jsonl-purged fallback) confirms zero client bytes in the payload while the plugin facts (skill/verdict/errorCode/toolFamily/repoKind/counts) still travel.
- **Adversarial review (7 independent reviewers across two rounds) caught one real leak, now fixed:** `validPluginVersion` gated `pluginVersion` by a regex SHAPE (`/^\d+\.\d+\.\d+(-\S+)?$/`) and echoed the match VERBATIM — the `-\S+` prerelease tail was an unbounded channel, so a client-controlled `"0.8.1-ghp_…"` / `"1.0.0-AcmeCorp"` (reachable via the DIAG "Plugin:" fallback line) reached the public upstream. Fixed by never echoing input — `validPluginVersion` now extracts ONLY the numeric `\d{1,4}.\d{1,4}.\d{1,4}` core and discards any suffix (→ `0.8.1`, or `"unknown"`), so the field is structurally digits+dots. Reviewers confirmed every OTHER containment channel closed (`inSet` exact-membership, `classifyError` never echoes, `struggle` Set-filter, `clampInt`, fresh-object rebuild vs prototype-pollution, the collector→reducer record-shape contract, and consent/no-auto-merge gating).
- **Second review round (5 reviewers) — containment held everywhere; the remaining fixes are signal-fidelity / test-integrity (NOT leaks):**
  - **Test-proof integrity (BLOCKER in the PROOF, not the code):** the property test's leak detector was `JSON.stringify(struct).includes(bad)`, which is blind to JSON escaping (a verbatim `C:\src\x.cs` serializes as `C:\\src\\…` → a raw-substring scan misses it) — a regression could reintroduce a backslash/quote/newline leak and stay green. Replaced with a recursive `assertNoLeak` over decoded string values + a strict-shape assertion (exact key set, so an unforeseen extra field fails) + injected the escaping class (`\ " \n \t`) into the adversarial set and the fuzz alphabet.
  - **Fingerprint dedup fragmentation (2a):** feedback counts were folded into the fingerprint unconditionally, so the SAME finding from two clients (one who also left feedback) forked into two upstream issues. Now feedback folds into the hash ONLY for a feedback-only report (no findings) — the "converge to ONE issue" invariant holds while feedback-only 👍/👎 stay distinct (D2).
  - **Batch double-count (5a):** `--batch` keyed on DIAG files, so two `DIAG-<sid>-*.md` for one session double-inflated occurrences/feedback/sessionCount. Now deduped by session id (every file still purged).
  - **Classifier ordering (B1):** `504 Gateway Timeout` → HTTP_5XX (was NETWORK_TIMEOUT), test-runner timeouts → TEST_FAILED, `✗ Build failed` → BUILD_FAILED, and a bare 3-digit count (`431 errors`) no longer reads as an HTTP status (HTTP codes now require an HTTP context word). NETWORK_TIMEOUT is errno/connection-scoped.
  - **Cross-path severity (B2):** the jsonl-purged fallback now DERIVES severity from the verdict (BROKEN→S1 / DEGRADED→S2) instead of trusting the DIAG's parsed `sev`, so the same defect fingerprints identically via the jsonl vs DIAG paths.
  - **Destination allowlist (A3):** `deliver` refuses any `--repo` not matching `VirtoCommerce/*` (the closed-schema body already carried no client bytes, but this stops an issue being misrouted to an arbitrary token-writable repo).
  - Minor: dropped the synthetic `(session)` placeholder from the fallback; batch keeps the newest `pluginVersion`; tightened the `toolFamily` github matcher. Tests added: classifier ordering, 2a/D2 fingerprint semantics, `mergeStructs` + `parseDiag` coverage (Gap 2).
- Trade-off (deliberate): no free-text root-cause/proposed-fix upstream — that richness stays in the LOCAL `DIAG-*.md`; a human attaches a sanitized repro when opening the PR (§2a human-in-loop). New error shapes fail-safe to `UNKNOWN`.
- `plugins/vc-fix` + `scripts/unit/` + `plugins/vc-fix/knowledge/diagnostics/` + docs only — the `.claude/` mirror stays on the pre-5509 model.
...
- **Broken `test:containment` npm script** — pointed at `tests/self-check-containment.test.mjs`, deleted earlier in this branch; repointed to the migrated `scripts/unit/upstream-reduce.test.mjs` + `deliver-feedback.test.mjs` (the closed-schema property/fuzz proof — the earlier `deliver-scrub.test.mjs` was dropped with the free-text scrubbers).
...
- **Fix 1 — Stop-hook over-defers on non-owned background tasks** (`hooks/session-telemetry.mjs`). The phantom "1 pending subagent" was a `run_in_background` **SHELL** task belonging to a **different** session that the platform surfaced in `ev.background_tasks`. The checkpoint-vs-terminal check trusted `background_tasks.length` **exclusively**, but that array is not session-scoped, so a foreign task forced perpetual deferral — the terminal finalize never ran, `flagged[]` was never computed, no command span closed, and no clean/finding line surfaced. Now we defer **only** when a pending background item is a genuine **agent** (never a shell/bash task) that is either matched to one of this session's own open agent ops by id/tool_use_id, **or** is agent-kind while we hold a **fresh** open agent op (covers a platform that keys an agent bg task by a distinct `agent_id` ≠ our tool_use_id, so a genuinely-running subagent is not finalized early — the freshness gate bounds it against defer-forever). Hard guard: **zero open agent ops ⇒ never defer** (finalize). The absent-field `freshOpenAgents` fallback + the STALL_MS orphan backstop are unchanged. When we do defer, `pendingSubagents` reflects OUR pending agents, not global background noise.
- **Fix 2 — command span not attributed for the capture-enabling command** (`hooks/session-telemetry.mjs`). `/project-init` turns capture ON mid-run (its §0b consent), so its own `/project-init` UserPromptSubmit fired while capture was still a no-op → no command span opened, and the run's tool-level failures could never be flagged. Now, when the terminal finalize sees a `complete --skill "<name>"` marker for THIS session but no command/skill span of any kind, it **synthesizes** a command span (start = session start, end = now), rolls the session's orphaned blocking-error counts onto it, and closes it — so the run is represented by one real span and any failed tool op within it becomes flaggable. Guarded to that exact blind spot (a plain-dev turn emits no `complete`, and the sid-scoped marker guard blocks cross-session bleed). NB: the roll-up uses session `totals`, which count self-corrected errors too, so a clean run that self-recovered a transient error may synthesize a `failed` (not `success`) span — the fail-toward-escalation direction, diagnostic noise not a break.
- **Fix 3 — delivery scrubber withheld plugin internals as if client data** (`skills/vc-self-check/deliver.mjs`). `containsClientShape()` flagged ANY compound camelCase (`/[a-z][A-Z]/`) as client-shaped, so the plugin's OWN symbols (`pendingSubagents`, `sawPluginSpan`, `anySkillSeen`, `openOps`, `roleStates`, `currentCommand`, `freshOpenAgents`, …) were withheld and every finding row was downgraded to "(client-specific evidence withheld)". Fix: a **plugin-symbol allowlist** is derived at delivery time by scanning the plugin's own source (`hooks/*.mjs`, `skills/**`, `commands/**`, `ci/lib/**`, bounded to `pluginRoot()`, cached) for declared identifiers and unioned with `SAFE_TERMS`; it is checked **before** the shape rules, so plugin code identifiers survive. The camelCase shape rule itself **stays BROAD** (`/[a-z][A-Z]/`) on purpose — client source identifiers are overwhelmingly lowercase-first camelCase (`orderSyncService`), so narrowing it to Capital-first would leak them to the PUBLIC upstream (a draft that did so was caught by three independent adversarial reviewers as a client-data-containment BLOCKER and reverted). **Client-data protection is unchanged:** the hard gates — configured client terms from `project-profile.json` (any case, incl. inside underscore/dot/hyphen-joined identifiers), path/URL/email/ticket/UNC/drive shapes, the broad camelCase identifier rule, and the shared `redact()` secret scrub (JWT/PAT/AccountKey/SAS/Basic-auth/PAN) — all stay fully intact and are applied first.
- Tests: `scripts/unit/session-telemetry.test.mjs` gains a Fix-1 defer/terminal ownership matrix (incl. the distinct-`agent_id` defer) + the Fix-2 synthesized-flagged-span case (and the two tests that encoded the old buggy behavior — `terminal (foreign background_task)…` and `opt-in mid-run…` — were updated to the corrected semantics). New `scripts/unit/deliver-scrub.test.mjs` covers the Fix-3 acceptance cells + a regression guard that a lowercase-first camelCase **client** identifier is withheld while the plugin's own symbols survive. 237/237 unit tests green.
