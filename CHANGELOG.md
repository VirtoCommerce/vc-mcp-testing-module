...
- **`deliver.mjs` rewritten** to build the outbound title/body SOLELY from `validateUpstream(reduce(...))`; the old text-derived `fingerprint`/`findingSig` were removed (superseded by `fingerprintStruct`). `redact()` / `scrubText` / `containsClientShape` **stay as local defense-in-depth** but the upstream path no longer DEPENDS on them. The collector `hooks/session-telemetry.mjs` is **unchanged** (capture stays opt-in, never throws). Consent (`feedback.mode`) + §2a route resolution unchanged.
