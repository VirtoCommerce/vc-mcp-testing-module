...
- **Secrets** (passwords, API tokens): `.env.playwright.local`, `.env.local` only — gitignored
...
**Product context — ask VirtoOZ before you guess:**
- **Before you WRITE a claim about how the platform or the storefront behaves** — knowledge file, oracle, bug report, test assertion, fix or verdict — **query VirtoOZ** (`/vc-docs`; VirtoOZ MCP, Context7 `/virtocommerce/vc-docs` fallback). **The trigger is the write, not whether you feel unsure:** converging repo + live + source evidence does not discharge it. Never substitute memory, inference, or "the code looked like it" for documented behaviour. **Grounding order:** this repo's knowledge first (`.claude/knowledge/domain/<slug>.md`, the `BL-*`/`ECL-*` oracles, `.claude/ROUTING.md`), VirtoOZ second, live/source last — but where the question is what the platform DOES rather than what it SHOULD, `kb_ask` (next bullet) comes before grepping an oracle and always before live/source, because it is the banked result of that live check; write back into the right knowledge file whatever VirtoOZ had to tell you. Docs are ONE source — a `BL-*`/`ECL-*` promotion still needs its 3-source bar (docs + live + source), and live behaviour contradicting the docs is a finding, not an error. **Released vs new decides the source:** RELEASED — VirtoOZ + our knowledge bind, and a released behaviour VirtoOZ does not document (or documents wrongly) is itself a **finding**, never a licence to guess and never "no requirement"; NEW — no doc page yet, so the ticket AC + source + the LIVE deployed build are ground truth (provenance `{DOC}`/`{BL}` vs `{SPEC}`/`{OBSERVED}`). Per-team teeth: `.claude/knowledge/agents/{qa,ba,developers}/shared-instructions.md` §Product behaviour / §Documentation source.
- **What the platform was OBSERVED to do is looked up, never grepped** — the tool is called **`mcp__kb__kb_ask`** (that exact id; **the `kb` tools are DEFERRED, so load them first: `ToolSearch` → `select:mcp__kb__kb_ask,mcp__kb__kb_capture,mcp__kb__kb_confirm,mcp__kb__kb_dispute`**), or **`npm run kb -- ask "<q>"`**, which costs no search hop, needs no MCP server and works on a fresh clone before any restart. It answers with trust and provenance; exit 1 = nobody has written it down (find out, then `kb_capture`), 3 = the base was NOT read. **Put the COORDINATE in the question** — the endpoint, the GraphQL operation name, the page path — because that is the signal the ranker is strongest on; a question in pure prose misses entries that are sitting right there. **Disagreeing is a verb, not a second entry:** what you saw contradicts an entry ⇒ `kb_dispute` it (a dispute retires nothing — it flags, and a human decides); what you saw matches ⇒ `kb_confirm` it, which is the only thing that makes the trust label mean anything. Capturing the same fact again instead is how the base grows two answers to one question. **The base is PUBLIC** (`VirtoCommerce/vc-knowledge`) — questions and claims land in a public repo, so nothing client-specific, credentialed or customer-named goes into one.
...
## Where the rules live — three loading tiers (and one that does not travel)
...
- **Loaded WHOLE when reached** (`.claude/commands/*.md`, `.claude/agents/*.md`, `.claude/skills/*/SKILL.md`): not a fourth location — a property of the three above. A command is paid in full the moment it is invoked, an **agent definition on every dispatch of that agent**, a `SKILL.md` whenever the skill loads; none can be read in part. So each has a **body budget** (BUDGET-004, below), and detail belongs in a supporting file the step reads only if it needs it. A skill's *supporting* files are exempt by design — being read selectively is what they are for.
...
**BUDGET-004 — the prompt-body budget (hard, per-file ratchet).** Every file in the loaded-whole tier is capped at **19,000 chars (~5k tokens** — the Level-2 figure BUDGET-003 already uses for `SKILL.md`). Before this, a prompt was budgeted by the *directory it sat in*: a 71K-char command was exempt while the same bytes in a `SKILL.md` were flagged, and `.claude/agents/` — re-paid on every dispatch — was measured by nothing at all. **A new or unlisted prompt over the cap fails the build.** The files already over it are recorded with their current size in `scripts/maintenance/.prompt-size-baseline.json` and **may only shrink**; an entry that drops under the cap, or whose file is renamed away, must be deleted (`npm run context:check:baseline` rewrites it — it can only ever record a shrink, never grant headroom). The budget's closing rule is the one above, unchanged: *move something down a tier, never raise the budget.*
...
### What reaches a teammate, and what does not

All three tiers above are **git-tracked**, so they reach every clone, every machine and every session with no plugin install and no per-user setup — that is the whole mechanism, and `.claude/settings.json` is tracked for the same reason. Three things are **not** tracked and so reach nobody but their author: `~/.claude/CLAUDE.md` (personal working preferences), the per-user memory directory under `~/.claude/projects/<project>/memory/`, and `.claude/settings.local.json`. **A rule that must hold for the team lives in a tracked tier or it does not exist.**

**A memory slug is not a citation.** Tracked files here cite private-memory slugs — `feedback_*` / `reference_*` / `project_*`, written as prose such as *"per memory `feedback_env_resilience`"* — in **522 places across 131 distinct slugs** (measured 2026-09-11). Each resolves to a file that exists on exactly one laptop, so to every other reader the pointer is dangling, and `context:check` does not catch it: `DOC-003`/`DOC-003E` ratchet **paths**, and a bare slug is not a path. So a memory slug is acceptable **only as provenance appended to a claim the tracked file already states in full** — never as the place the claim lives. A citation that carries the claim instead of following it is a defect: promote the fact into the topical `.claude/knowledge/**` file and cite that path.
