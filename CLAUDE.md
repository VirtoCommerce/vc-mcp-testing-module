...
npm run ci:audit         # Test-case staleness audit — one suite/run, 3-axis triangulation, draft PR (ci:audit:dry = pick only)
npm run tc:audit:queue   # Which suite is most overdue for triangulation (risk tier → oldest Audited: stamp)
...
│   ├── commands/         #   29 slash commands, flat *.md (incl. /project-init onboarding)
│   ├── agents/           #   19 agents (QA + BA + Developers teams). FLAT *.md, non-recursive discovery
│   ├── skills/           #   38 skills, each skills/<name>/SKILL.md (ONE level; no category subfolders)
...
├── ci/                   # CI regression — Docker + Claude Agent SDK (TRACKED, not gitignored — see the Gitignored note below)
...
- `.claude/rules/skills-commands.md` — 29 commands + 38 skills with arguments
...
- **`/qa-test` is a gated + independently-verified lifecycle** — every step runs `DOER → GATE → INDEPENDENT VERIFIER`. The verifier is a **fresh `qa-lead-orchestrator` instance in §Verifier Mode** (`.claude/agents/qa-lead-orchestrator.md`), never the pipeline's inline orchestrator and never the step's own doer: it re-derives evidence from source (re-runs `suites:review`/`td:validate`/`compute-metrics.ts --gate feature`, re-opens evidence, delegates a live re-check to a specialist on a **different browser lane**), returns APPROVE/REJECT, and on REJECT gives reason + fix → the doer fixes → re-verify (≤2, then STOP). The step gates: 1 *Test Model complete* · 3 *Artifacts reviewed + data seeded* (hard STOP) · 4 *Execution evidenced + risk areas explored* (exploratory folded into execution) · 5d–5e *Triage + verdict sound* (hard STOP before filing) · 5h *Feature Release Gate ratified* (`skills/qa-metrics/quality-gates.md` §1a). Human stays terminal (TESTED/REOPEN + a GO/NO-GO recommendation). The Feature Release Gate's pass-rate/bug math is deterministic via `scripts/regression/compute-metrics.ts --gate feature`. **Test cases are one pipeline shared with `/qa-test-lifecycle`, not two:** `/qa-test` Step 3 runs the same author → review → auto-fix → provision mechanism as lifecycle Phases 3–4 (owned by the `/qa-test-cases-generator` + `/qa-review-tests` + `/qa-generate-data` skills — neither command restates a dimension, code, column or enum), differing only in that its rows land in a **run-scoped** `reports/tickets/{SPRINT}/VCST-XXXX/test-cases.csv` that no runner reads. Promotion into durable `regression/suites/` coverage is **`/qa-test-lifecycle` Phase 6P** and nowhere else: `/qa-test` 5i harvests its own execution as the `--verify` evidence that grounds assertions to `{OBSERVED}`, 5j hands off (`/qa-test-lifecycle VCST-XXXX --promote-only`), and 6P re-derives G10 eligibility from the CSV itself (never trusting the hand-off record) before an approval-gated `suites:append` + `suites:sync` flips the eligible cases `Draft → Reviewed`. Skipping the hand-off turns new coverage into a one-shot.
