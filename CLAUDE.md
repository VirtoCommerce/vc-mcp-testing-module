...
- **What the platform was OBSERVED to do is looked up, never grepped** — `kb_ask` (or `npm run kb -- ask "<q>"`) answers with trust and provenance; exit 1 = nobody has written it down (find out, then `kb_capture`), 3 = the base was NOT read.
