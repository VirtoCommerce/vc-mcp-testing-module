...
**Self-diagnostics (`vc-fix`, opt-in).** A passive collector (`hooks/session-telemetry.mjs`, active only when `project-profile.json` sets `selfDiagnostics: true`) records outcome-classified span + observation records to `<outputRoot>/.vc-fix/diagnostics/<sid>.jsonl`; `/vc-self-check` judges them against `.claude/knowledge/diagnostics/skill-expectations.md`; `deliver` files a consent-gated GitHub Issue built ONLY from a closed-vocabulary struct (no free text, no client names — `upstream-reduce.mjs`, `plugins/vc-fix/knowledge/diagnostics/upstream-schema.md`). Capture never decides what matters; the collector, `/vc-self-check` and the `Stop` hook are three separate layers. Local artifacts are ephemeral (log → analyze → contribute → delete). The four core files are byte-identical in `plugins/vc-fix/` and `.claude/` (CI-enforced). Full design rationale and incident history: `docs/decisions/self-diagnostics-design.md`.
...
- **When how the platform or the storefront is SUPPOSED to behave is unclear, query VirtoOZ first — `/vc-docs` (VirtoOZ MCP; Context7 `/virtocommerce/vc-docs` fallback) — before acting on it.** This is the default, not a last resort: a verdict, a bug report, a test case, a fix or a knowledge edit built on guessed product behaviour is wrong in the one direction nobody re-checks, because it reads as confident. Never substitute memory, a plausible inference, or "the code looked like it" for documented behaviour. Docs are ONE source — a `BL-*`/`ECL-*` promotion still needs its 3-source bar (docs + live + source), and live behaviour that contradicts the docs is a finding, not an error. **Grounding order — every agent, every team:** this repo's own knowledge first (`knowledge/domain/<slug>.md`, the `BL-*`/`ECL-*` oracles, `.claude/ROUTING.md`), VirtoOZ second, live/source last; what VirtoOZ had to tell you that the repo should have known goes back into the right knowledge file, so the next run reads it instead of re-asking. **Released vs new decides the source:** a RELEASED feature is documented — VirtoOZ + our knowledge bind, and a released behaviour VirtoOZ does not document (or documents wrongly) is itself a **finding**, never a licence to guess and never "no requirement"; a NEW feature has no doc page yet, so the ticket AC + source + the LIVE deployed build are the ground truth (provenance `{DOC}`/`{BL}` vs `{SPEC}`/`{OBSERVED}`). Per-team teeth: `.claude/knowledge/agents/{qa,ba,developers}/shared-instructions.md` §Product behaviour / §Documentation source.
...
**A path starting `knowledge/` is in the KNOWLEDGE BASE, not this repository.** The oracles, domain maps, API and architecture references describe the PLATFORM — what makes them untrue is a change out there — so they live in `VirtoCommerce/vc-knowledge` and travel with it. Fetch it once per machine with **`npm run kb -- sync`** (`~/.claude/vc-knowledge`; there is no bare `kb` on PATH, and `npx kb` is somebody else’s package); `npm run bl:extract -- --domain <d>` slices the invariants you need without naming a path at all. A path starting `.claude/knowledge/` is still this repository's: how WE run, not how the platform behaves.

**`knowledge/oracles/business-logic.md` is GENERATED and is never hand-edited** — the 217 `BL-*` invariants are records in the base's normative plane, the page is rendered from them (`npm run bl:render`) and CI byte-compares it (`bl:render:check`), so an edit made on the page is lost at the next render and fails that gate on somebody else's commit. Read it freely; to CHANGE one, edit the rule's record — `kb show <BL-id>` prints its path — and re-render. The procedure, including adding and retiring a rule: `.claude/skills/qa-review-oracles/bl-audit-criteria.md` §4.
...
| **What a domain IS** — actors, value chain, surface inventory per layer, where the layers DISAGREE, coverage shape | `knowledge/domain/<domain>.md` (shape: `domain-map.md`; build/refresh `/qa-domain-map <slug>`; freshness `npm run domain:check`) |
...
| Oracles — BL invariants, ECL edge cases, bug catalog; their audit + value gate | `knowledge/oracles/`, `.claude/skills/qa-review-oracles/` (`npm run oracles:rank`, `bl:lint`, `ecl:lint`) |
...
| Why any of the above is shaped the way it is | `docs/decisions/qa-test-evolution.md`, `docs/decisions/self-diagnostics-design.md`, `docs/decisions/regression-history.md`, `docs/decisions/mirror-parity.md`, `docs/decisions/unit-test-roi.md` |
