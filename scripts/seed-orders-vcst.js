 ...
 * Order Seed — vcst-qa
 *
 * Creates 20 orders for John Mitchell (TechFlow contact, originally seeded as
 * AcmeCorp 2026-03-10 then reassigned to TechFlow 2026-05-08) on the vcst-qa
 * B2B-store, using a varied pool of products discovered live from the
 * storefront catalog via xAPI GraphQL.
 *
 ...
 *
 * Loads .env.vcst for URLs/admin user, .env.local for ADMIN_PASSWORD.
 * Writes IDs/SKUs to test-data/b2b/_seed-results-orders-vcst.json and a
 * markdown report alongside it.
 *
 * Usage:
 *   node scripts/seed-orders-vcst.js --dry-run [--verbose]
 *   node scripts/seed-orders-vcst.js          # real run
 *   node scripts/seed-orders-vcst.js --force  # overwrite existing results
 */
...
// override:true matches config.js — needed because the IDE shell pre-sets ADMIN
// from a previous session, which would otherwise win over .env.vcst.
config({ path: join(ROOT, '.env.vcst'), override: true });
config({ path: join(ROOT, '.env.local'), override: true });

const BACK_URL = process.env.BACK_URL;
// vcst-qa: ADMIN=admin (default), ADMIN_PASSWORD in .env.local — no _VIRTO_ fallback needed.
...
const RESULTS_FILE = join(ROOT, 'test-data', 'b2b', '_seed-results-orders-vcst.json');
const REPORT_FILE = join(ROOT, 'test-data', 'b2b', `seed-report-orders-vcst-${DATE_STAMP}.md`);

// ---------- TARGET CUSTOMER (vcst-qa) ----------
// John Mitchell — contact id and user id from _seed-results-orgs.json (2026-03-10 seed).
// Organization reassigned 2026-05-08: contact moved from AcmeCorp to TechFlow,
// so future order seeds must use TechFlow's id. (Also see fix-john-organization-vcst.js
// which back-filled this on the 20 already-seeded May orders.)
const CUSTOMER = {
  userId: '143bc845-7ba3-4982-ae9a-a9446a399705',
  contactId: 'd0f765ba-3d2d-4f4e-a4b4-e3306e153178',
  userName: 'test-john.mitchell-20260310@test-agent.com',
...
async function main() {
  console.log(`\n📦 Order Seed — vcst-qa${DRY_RUN ? ' [DRY RUN]' : ''}`);
