...
import {
  PRESET_SOURCES, requiredPresetNames, mergeAllowedValues, fileNameCollisions,
  buildDictionaryUpdateBody, classifyPresetName, presetNameToFileName,
  mergeStoreWhiteLabeling, fieldsLostByWlWrite,
} from '../seed-data/store/theme-preset-specs.mjs';

/** The platform-global WL dictionary the theme presets live in (vc-module-white-labeling). */
const THEME_PRESET_SETTING = 'WhiteLabeling.ThemePresetNames';

/**
 * The preset names vc-frontend bundles, generated by `npm run presets:sync` and drift-guarded by
 * `presets:check`. Absent (or stale) is survivable — every consumer treats a name outside the list
 * as "check this", never as an error — so a missing file degrades to an empty list, not a throw.
 */
function bundledPresets() {
  try {
    const p = join(ROOT, 'scripts/lib/theme-presets.generated.json');
    return existsSync(p) ? (JSON.parse(readFileSync(p, 'utf8')).presets || []) : [];
  } catch { return []; }
}

/**
 * Warn when a preset name is not one vc-frontend bundles AND the live storefront does not serve it
 * either — the storefront will silently fall back to the theme default, so a case asserting on this
 * theme is asserting on the default one and still passes. Two sources because NEITHER is sufficient
 * alone: bundled presets are compiled into the JS and 404 as files, and a deployment can ship extra
 * presets as files that no vc-frontend ref knows about. Advisory only — it never blocks a seed.
 */
async function warnIfPresetUnresolvable(name, where) {
  const verdict = classifyPresetName(name, bundledPresets());
  if (verdict.verdict === 'RESOLVES') { verbose(`theme preset "${verdict.name}" (${where}): ${verdict.reason}`); return; }
  const front = (process.env.FRONT_URL || '').replace(/\/+$/, '');
  if (front) {
    try {
      const r = await fetch(`${front}/assets/presets/${presetNameToFileName(name)}.json`);
      if (r.status === 200) { verbose(`theme preset "${verdict.name}" (${where}): served by this deployment as /assets/presets/${verdict.fileName}.json`); return; }
    } catch { /* unreachable storefront is not evidence either way */ }
  }
  log(`  ⚠ theme preset "${verdict.name}" (${where}) may not resolve — ${verdict.reason}.`);
  log('    The storefront falls back to the theme default SILENTLY, so a case asserting this theme would still pass.');
}
...
/**
 * Ensure the platform's WHITE-LABELING THEME PRESET dictionary offers every preset this repo's
 * fixtures name. REQUIRED for the same reason ensureCurrencies is: a store (or an org) can NAME a
 * theme preset, but `WhiteLabeling.ThemePresetNames` is the closed list both WL blades select from
 * — and vc-module-white-labeling ships it with exactly one value, "Default". On a from-scratch env
 * nothing can be set to Coffee/Watermelon/black-gold/Purple Pink at all, by hand or by seeder.
 *
 * Platform-GLOBAL, not store-scoped (objectType/objectId null) — see theme-preset-specs.mjs, which
 * holds all of the logic and the rationale. Idempotent, and ADDITIVE ONLY: the live dictionary is
 * read, the fixtures' names are merged in, and the write is skipped entirely when nothing is new.
 * `POST /api/platform/settings` REBUILDS the value set from `allowedValues`, so a blind write would
 * delete operator-added presets this repo has never heard of.
 */
export async function ensureThemePresetNames(api) {
  const rowsByFile = Object.fromEntries(PRESET_SOURCES.map((s) => [s.file, loadCsvOptional(s.file)]));
  const { names } = requiredPresetNames(rowsByFile);
  if (!names.length) { verbose('no theme_preset referenced by any fixture — nothing to register'); return; }

  const live = await api('GET', `/api/platform/settings/${THEME_PRESET_SETTING}`, null, { expectStatus: [200, 404] });
  if (!live?.name) { log(`  ⚠ ${THEME_PRESET_SETTING} not registered on this platform — is vc-module-white-labeling installed? Skipping.`); return; }

  const { next, added } = mergeAllowedValues(live.allowedValues, names);
  // Two spellings the storefront cannot tell apart ("Purple Pink" vs "purple-pink") both resolve to
  // ONE preset — report, never auto-delete; removing a dictionary value is a human decision.
  for (const c of fileNameCollisions(next)) {
    log(`  ⚠ theme presets ${c.names.map((n) => `"${n}"`).join(' and ')} both resolve to preset "${c.fileName}" — the storefront renders them identically`);
  }
  for (const n of names) await warnIfPresetUnresolvable(n, 'fixture theme_preset');
  if (!added.length) { verbose(`theme presets already registered (${next.length}): ${next.join(', ')}`); return; }
  if (DRY_RUN) { log(`[DRY] would add ${added.length} theme preset(s): ${added.join(', ')} (keeping ${live.allowedValues?.length || 0} live)`); return; }

  await api('POST', '/api/platform/settings', buildDictionaryUpdateBody(live, next), { expectStatus: [200, 204] });
  // Read back: a dictionary write that silently no-ops leaves the WL blades with a list the
  // fixtures cannot select from, and nothing downstream would say so — the case just picks Default.
  const after = await api('GET', `/api/platform/settings/${THEME_PRESET_SETTING}`, null, { expectStatus: [200] });
  const missing = added.filter((n) => !(after?.allowedValues || []).some((v) => String(v).toLowerCase() === n.toLowerCase()));
  if (missing.length) { log(`  ⚠ theme preset(s) did not persist: ${missing.join(', ')} — the WL blades will not offer them`); return; }
  log(`  ✓ Registered ${added.length} theme preset${added.length === 1 ? '' : 's'}: ${added.join(', ')} (${after.allowedValues.length} total)`);
}

/**
 * Apply the store's declared `theme_preset` (stores.csv) to its STORE-LEVEL white-labeling record —
 * the thing that decides what the storefront actually renders for everyone who is not in an org with
 * its own override. The column has existed since stores.csv was written and nothing read it, so the
 * declared store theme was fiction; this makes it real.
 *
 * Merged, never composed: the live record carries `logoUrl` and both link-list names, which
 * stores.csv cannot supply — a CSV-shaped body would save cleanly and blank all three
 * (theme-preset-specs.mjs §PUT REPLACES). The write is refused outright if that would happen.
 * Only `themePresetName` ever moves: `isEnabled` is left alone on an existing record, because a
 * fixture declaring a theme is not a decision to switch somebody's white labeling back on.
 */
export async function ensureStoreWhiteLabeling(api, { storeId = STORE_ID, row = null } = {}) {
  const theme = (row?.theme_preset || '').trim();
  if (!theme) { verbose(`no theme_preset declared for ${storeId} — store-level WL record untouched`); return; }

  // 200-with-a-null-body is how this route says "no record yet" (verified live against B2C-store),
  // so 404 is NOT that — it means the route is absent, i.e. vc-module-white-labeling is not
  // installed. Distinguishing them matters: treating a 404 as "no record" would POST into a route
  // that does not exist and take the whole store phase down on a platform that simply lacks the module.
  let live;
  try {
    live = await api('GET', `/api/white-labeling/store/${encodeURIComponent(storeId)}`, null, { expectStatus: [200, 204] });
  } catch (e) {
    log(`  ⚠ store-level WL record skipped — GET /api/white-labeling/store/${storeId} failed (${String(e.message).slice(0, 120)}). Is vc-module-white-labeling installed?`);
    return;
  }

  const { next, op, from } = mergeStoreWhiteLabeling(live, {
    storeId, themePresetName: theme, isEnabled: !/^false$/i.test(String(row?.whitelabeling_enabled ?? 'true')),
  });
  if (op === 'noop') { verbose(`store-level theme preset already "${theme}" on ${storeId}`); return; }
  await warnIfPresetUnresolvable(theme, `${storeId} store-level WL record`);

  // The 2026-08-27 guard, applied to this entity: nothing the live record holds may come back empty.
  const lost = fieldsLostByWlWrite(live, next);
  if (lost.length) {
    log(`  ⚠ REFUSED store-level WL write on ${storeId}: it would blank ${lost.join(', ')}. PUT /api/white-labeling REPLACES the record.`);
    return;
  }
  if (DRY_RUN) { log(`[DRY] would ${op} store-level WL theme on ${storeId}: ${from ? `"${from}" → ` : ''}"${theme}"`); return; }

  if (op === 'create') await api('POST', '/api/white-labeling', next, { expectStatus: [200, 201] });
  else await api('PUT', '/api/white-labeling', next, { expectStatus: [200, 204] });

  // Read back: a WL write that no-ops leaves the storefront on the OLD theme while the seed log
  // claims the new one — and the difference is a colour, which no assertion in this repo checks.
  const after = await api('GET', `/api/white-labeling/store/${encodeURIComponent(storeId)}`, null, { expectStatus: [200, 204, 404] });
  if ((after?.themePresetName || '') !== theme) {
    log(`  ⚠ store-level theme preset reads back as "${after?.themePresetName ?? '(none)'}", expected "${theme}"`);
    return;
  }
  const blanked = fieldsLostByWlWrite(live, after);
  if (blanked.length) { log(`  ⚠ ${blanked.join(', ')} is now empty on ${storeId}'s WL record — restore it (logo/link lists are not in stores.csv)`); return; }
  log(`  ✓ Store-level WL theme on ${storeId}: ${from ? `"${from}" → ` : ''}"${theme}"`);
}
...
/**
 * Activate the store's shipping + payment methods named in stores.csv (`shipping_methods` /
 * `payment_methods`, semicolon-separated method CODES).
 *
 * These are NOT fields on the store entity — GET /api/stores/{id} carries no shippingMethods or
 * paymentMethods key at all. They are per-store sub-resources: the platform pre-registers one row per
 * installed provider with isActive=false, and activating one is a PUT of that row. So this runs AFTER
 * the store exists and never touches the store body.
 *
 * Why it exists: without it a fresh DB gets a store that cannot check out — every method sits
 * isActive=false, and any fixture that places an order dies on "no available shipping method"
 * (measured on localhost 2026-09-15, seed:missions-e2e). Idempotent: an already-active method is left
 * alone. An unknown code warns rather than throws — a provider module may simply not be installed.
 */
export async function applyStoreMethods(api, storeId, row) {
  const csvList = (s) => (s || '').split(';').map((x) => x.trim()).filter(Boolean);
  const want = { shipping: csvList(row?.shipping_methods), payment: csvList(row?.payment_methods) };
  if (!want.shipping.length && !want.payment.length) return false;
  let changed = false;
  for (const [kind, path] of [['shipping', '/api/shipping'], ['payment', '/api/payment']]) {
    if (!want[kind].length) continue;
    if (DRY_RUN) { log(`  [DRY] would activate ${kind}: ${want[kind].join(', ')}`); continue; }
    const res = await api('POST', `${path}/search`, { storeId }, { expectStatus: [200] });
    const rows = res?.results || res || [];
    for (const code of want[kind]) {
      const m = rows.find((r) => r.code === code);
      if (!m) { log(`  ⚠ ${kind} method ${code} is not registered on this platform — skipped`); continue; }
      if (m.isActive) { verbose(`${kind} ${code} already active`); continue; }
      m.isActive = true;
      if (m.storeId == null) m.storeId = storeId;
      try {
        await api('PUT', path, m, { expectStatus: [200, 204] });
        log(`  ✓ ${kind} method activated: ${code}`);
        changed = true;
      } catch (e) { log(`  ⚠ ${kind} ${code} activation failed: ${String(e.message).slice(0, 120)}`); }
    }
  }
  return changed;
}
...
export async function ensureStore(api, { storeId = STORE_ID, catalogId = null, existing = undefined } = {}) {
  ...
  // Currencies are a prerequisite whether we create or update the store. So is the platform's
  // theme-preset dictionary: it is what the store-level and org-level WL blades select from, and
  // it is platform-global, so it is registered once regardless of which store this call targets.
  await ensureCurrencies(api, currencies);
  await ensureThemePresetNames(api);
  ...
  if (store?.id) {
    ...
    // Shipping/payment live in their own sub-resources, so this is deliberately OUTSIDE the
    // `changed` guard above — a store whose body is already correct can still have every method
    // inactive, which is exactly the fresh-DB case.
    await applyStoreMethods(api, storeId, row);
    await ensureStoreWhiteLabeling(api, { storeId, row });
    return store.id;
  }
