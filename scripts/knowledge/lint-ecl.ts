...
export const SECTION_RE = /^###\s+(\d+)\.(\d+)\s+(.*?)\s*$/;
/** `## 13. Business Logic & Workflow Edge Cases` */
export const CHAPTER_RE = /^##\s+(\d+)\.\s+(.*?)\s*$/;
...
export const APPENDIX_RE = /^##\s+Appendix\s+D\b/i;
...
function main(): void {
  ...
  const file = argv.find((a) => !a.startsWith("--")) ?? knowledgePath("oracles/e-commerce-edge-cases-library.md");
