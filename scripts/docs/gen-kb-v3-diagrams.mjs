...
function pageOverview() {
  ...
  p.card("AGT", 380, 240, 380, 190, "Агент-помощник", [
    ...
    "подтверждает механизму то, чем воспользовался",
    "оспаривает запись, если она расходится с продуктом",
  ...
  p.card("WIN", 380, 470, 380, 210,
    ...
    "выдаёт одно текущее состояние — заменённое в ответ не берёт",
    "показывает обе стороны, если проект переопределил платформу или если запись оспорена",
    "сообщает агенту, насколько ответу верить",
  ...
  p.card("KBC", 880, 505, 360, 260,
    ...
    "переопределяет у себя правила платформы",
    "отменяет у себя неприменимые правила платформы",
    "дополняет правила платформы своими",
    "ставит спор, когда переопределённое правило изменилось",
    "не пишет в базу платформы",
  ...
  p.card("KEEP", 380, 720, 380, 320,
    ...
    "засчитывает повтор факта в подтверждение записи, новую не создаёт",
    "поднимает доверие записи до высшего сам, без человека",
    "читает источники: при их изменении перепроверяет сразу, остальное — по очереди",
    "помечает заменённую запись и убирает её из ответов, историю оставляет",
    "ставит спор на противоречащие записи и разрешает его проверкой",
    "не трогает закреплённое оператором и не возвращает снятое им",
    "пишет только в свою базу — из проекта в базу платформы не пишет",
    "публикует оператору сводку изменений",
  ], ...C.keeper, 52);

  const opBullets = [
    "снимает неверную запись — след остаётся",
    "закрепляет запись — механизм её не правит и не снимает",
    "ставит запись под сомнение: механизм проверит заново",
    "задаёт механизму правила повышения доверия",
