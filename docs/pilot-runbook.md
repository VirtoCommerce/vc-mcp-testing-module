...
At the kickoff (60 min), we'll:
  - Walk through `/project-init` together (5 min)
...
| 0:10 | They run `/project-init` | Did they answer every prompt without asking you? Which prompts confused them? |
| 0:25 | They run `npm run env:check` | Did it pass first try? If not, what was the blocker? |
...
Customer commits to:
1. Configure a SECOND env (`/project-init (for env `<other>`)`)
...
- Customer-onboarding command: [`../commands/qa-onboarding.md`](../.claude/commands/qa-onboarding.md)
