...
| | |
|---|---|
| **Status** | **Proposed (Revised)** — updated with P2.5 Seed Migration, Two-Tier Cadence, AST Anchors, and Doc Intake |
| **Date** | 2026-09-01 (Revised 2026-09-03) |
...
**This base starts without grandfathered format debt.** It does not carry forward
legacy unstructured prose or unverified claims: no format is inherited blindly, no
unverified identifier is grandfathered. However, to avoid the catastrophic
"empty-brain death spiral" (Risk 10), platform bootstrapping pairs deterministic code
extraction with a **P2.5 Seed Migration Pipeline** (§11.3): existing platform invariants
(211 Business Rules and 54 Edge Case sections) are audited and verified against live QA
suites (042, 078, 049) to enter with empirical evidence from day zero. Client bases
start empty from source deltas and intake client architecture documents via `/project-init` (§11.3).
v3 begins from the core architectural question — **what does an agent need in order to
answer a question about this platform correctly, and how does that answer stay correct?**
Everything below follows from that question, and entries are continually enriched by
the bootstrap extractors (§11.3) and by agents doing real work (§10.2).
...
| # | Error | v3 answer |
|---|---|---|
| 0 | **Framing: designed as a rescue of the existing corpus.** v1's context was that corpus's defects, so its entry model, its ids and its indexing were all shaped by what happened to be on disk — a migration wearing an architecture's clothes | The base starts clean without format debt (§1 opening, §2 Q0). Platform seeds are admitted only via the P2.5 Seed Migration pipeline with empirical QA verification (§11.3), while client bases start empty and use Doc Intake. Requirements come from what an agent needs to answer and keep correct |
...
| # | Theme | Decision |
|---|---|---|
| 0 | Starting point | **A clean base with verified seed migration (P2.5).** No legacy format inheritance or grandfathered identifiers. Platform seeds existing high-value BL/ECL via automated QA suite verification (P2.5); client base starts empty from code delta + Doc Intake (§11.3) |
...
| 9 | Re-verification initiative | **Two-tier cadence**: (1) Continuous Fast Consolidation in CI (minutes) turns drafts into id-minted entries immediately usable in search; (2) **Release-Gated Verification** (on Git tags) regenerates the Derived plane, re-checks anchors against release diffs, and corroborates business rules with standard QA release suites |
...
| 19 | Client bootstrap | Yes — same extractors over client repos + delta vs platform + **Doc & Context Intake** in `/project-init` (intake of customer Swagger/OpenAPI, Confluence exports, PRDs) |
| 20 | Isolation & promotion | Isolation by type (unchanged §2a). Client→platform promotion = a **fixed-format GitHub issue** with **mandatory Human-in-the-loop Diff Preview** before filing (§10.4), ingested into platform `drafts/` via bot; zero auto-flow of client secrets |
...
| Field | What | Why |
|---|---|---|
| `anchors[]` | Machine-checkable pointers into sources: `{repo, ref: <commit>, path, symbol?, symbolSignature?, contractHash}` — semantic AST coordinates (Roslyn for C#, ts-morph for TS) tracking the public symbol signature and contract hash, NOT raw line text | The drift sensor for experiential/derived-adjacent knowledge: when regeneration diffs touch an anchor's public contract, the entry is flagged for re-check (§6.4). Cosmetic code formatting, comment changes, and internal method refactorings do not trigger false drift. In TMS terms this is the entry's **justification** ([R§1]) |
...
### 5.2b The Test Suite Invariant: Prevention of Bug Legalization
A critical failure mode in autonomous knowledge capture is **bug legalization**: an agent
observes a defective system behavior (e.g. cart shipping fee mistakenly calculating as 0.00
due to a bug in an unmerged PR or staging build), observes that the code matches the live
behavior, and records it as a platform `behavior`. Over time, un-disputed usage would promote
this defect to `confirmed`, permanently blinding future test suites.

To prevent this without introducing human approval bottlenecks:
- **Cross-check against Regression Suites (`regression/suites/`):** An agent is **strictly
  forbidden from creating a `behavior` entry if the observed behavior contradicts an existing
  assertion in the repository's test suites**.
- If observed behavior contradicts an existing test assertion, the agent must treat it as a
  defect and invoke `/qa-bug`.
- In ambiguous cases where no test suite covers the area, the capture writes the entry with a
  provisional flag `suspected-defect: true`, excluding it from confident search results
  until corroborated by an official release verification pass.
...
- **Morphology and Query Language Discipline:** To prevent zero-hit false MISSes on inflected
  Russian queries (where `"расчет налогов"` would fail to match `"налог рассчитывается"` without
  stemming), the resolver enforces **English-first query canonicalization** by default (translating
  or normalizing domain queries to English, matching Virto Commerce's 100% English codebase and API).
  Alternatively, `vendor/minisearch.js` embeds a lightweight, zero-dependency Snowball Russian
  stemmer (~100 lines) within `processTerm`.
...
Append-only `drafts/`, stored as timestamped UUID files (`drafts/<timestamp>_<uuid>.jsonl`)
to ensure conflict-free parallel git commits across concurrent agent sessions, repeat sightings merge
...
### 6.1a Two-Tier Consolidation Cadence: Fast In-Sprint Pass vs Release Gate
To ensure knowledge accumulates continuously without delay while keeping verification
authoritative and noise-free, the lifecycle is decoupled into two tempos:

1. **Continuous Fast Consolidation (Minutes, In-Sprint):**
   - Runs in CI on every push touching `drafts/` (or locally via CLI).
   - Fast, deterministic, zero-LLM Node.js pass (~1-2 seconds).
   - Validates closed schema, runs MiniSearch near-duplicate check, **mints the eternal ID
     (`KB-BEHAVIOR-NNN`)**, and rebuilds `knowledge-index.json` and `catalog.md`.
   - **Knowledge enters active circulation immediately**: the entry is assigned status
     `entry` with label `unconfirmed` (medium-low trust). Other agents find it in search
     and cite it via `@kb(id)` within minutes of capture, preventing the "empty-brain death spiral".

2. **Release-Gated Verification (Milestones, on Git Tags):**
   - Runs when an official release tag is cut for a component (`vc-platform`, `vc-module-*`, `vc-frontend`).
   - Extractors regenerate the Derived plane against the immutable release tag.
   - Diff sensors detect contract and anchor changes against the prior release tag.
   - **Zero Extra Effort:** Reuses the results of the standard QA release regression run
     (Smoke 042, 078, critical suites). Invariants confirmed by green test runs receive
     corroboration score boosts to reach the **`confirmed`** label. Invariants broken by
     the release trigger Release Gate alerts for team triage.

**Why not heavy verification the moment an agent writes it.** Three reasons, and each of them is a gate
...
1. The client side builds a **GitHub issue from a fixed issue-form**: structured
   ...
   **Mandatory Interactive Diff Preview (Human-in-the-Loop):** Before dispatching the
   network request to GitHub, `kb promote` presents a terminal preview modal
   (`AskUserQuestion`) showing the exact sanitised markdown payload. The operator
   must explicitly confirm that no proprietary customer logic, client URLs, or credentials
   are exposed before the issue is submitted.
...
Platform brain:
1. Run extractors over `vc-platform`, `vc-module-*`, `vc-frontend` → derived tables +
   aggregate entries (hundreds, not thousands — the "would an agent re-ask this every task"
   selection test; full inventories stay tables).
2. **P2.5 Seed Migration Pipeline (`kb-migrate-legacy`):** Migrates the existing 211
   Business Rules (`business-logic.md`) and 54 Edge Case sections (`ecl`) into typed
   `behavior` and `pitfall` entries. It runs the canonical test battery (Smoke 042, 078, 049)
   against `vcst-qa`; rules verified by green test executions receive `evidence: live` stamps
   and enter the base with valid baseline weight, arming the platform brain on day zero.
...
- `/project-init` finds-or-scaffolds the knowledge repo (marker: `brain.json`), runs the
  ...
  `project-overview` structure entry ("what this deployment is, how it differs").
- **Doc & Context Intake Gateway:** `/project-init` provides an optional prompt allowing the
  operator to supply paths or URLs to existing client architecture documentation (OpenAPI/Swagger
  specs of ERP/PIM integrations, Confluence/Notion markdown exports, PRDs). `kb-bootstrap` parses
  these into initial client contracts and domain decisions with `provenance: client-doc`,
  giving client agents immediate context on the customer's enterprise topology.
...
**Fail-Open Contract for `vc-fix`:** In accordance with `vc-fix`'s core mandate of autonomous
bug fixing, integration with `vc-kb` is designed for **Graceful Degradation (Fail-Open)**. If
`vc-kb` is not installed, uninitialized, or encounters an internal fault, `/qa-fix` logs
`[KB: bypassed, plugin not available]` and proceeds with bug triage, reproduction, and fixing
without interruption. Knowledge enrichment and capture are advisory; they never block bug delivery.
...
| **P2** | **`VirtoCommerce/vc-knowledge` repository created**: layout, `brain.json`, taxonomy, the gates wired into its own CI | the gates run green on an empty corpus (a gate that only works once there is content is not a gate) |
| **P2.5** | **Seed Migration Pipeline (`kb-migrate-legacy`)**: Automated ingestion and test-battery verification of existing 211 BL and 54 ECL rules; verified entries admitted with empirical evidence and baseline weight | `npm run kb:migrate-check` exits 0; verified BL invariants findable via `resolveId` |
| **P3** | **The base fills itself**: extractors + `--check` + diff-event stream, bootstrap over `vc-platform` / `vc-module-*` / `vc-frontend` (§11.3), continuous fast consolidation in CI, regression goldens seeded, first capture call sites live in `vc-fix` | extractor unit tests pass; first agent-captured entry survives fast consolidation |
...
0. **The base starts clean with verified seed migration (P2.5)** — no grandfathered
   format debt; platform brain seeds existing high-value BL/ECL via empirical QA suite
   verification (P2.5), while client brains start empty from code deltas and Doc Intake (§11.3).
