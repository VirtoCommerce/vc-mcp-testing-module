...
sequenceDiagram
    ...
    Orch->>V: GATE 4 · re-open evidence, re-run 1 case on a different lane
    V-->>Orch: APPROVE (REJECT: PASS with no evidence to fix)
    ...
    Orch->>V: GATE 5 · every risk area probed?
    V-->>Orch: APPROVE (REJECT: run the skipped risk area)
    ...
    Orch->>V: GATE 6 · re-classify sample (live repro, diff lane), ratify verdict
...
sequenceDiagram
    ...
    opt Step 3 authored new cases (new_cases_authored > 0)
        Orch->>Orch: 6i Harvest Step 4 as --verify evidence (HYPOTHESIS to OBSERVED)
        Orch->>V: GATE 6i · re-run suites:review, re-open the artifact behind each OBSERVED
        V-->>Orch: APPROVE (REJECT: ungrounded OBSERVED or invented value to re-harvest)
        Orch-->>User: 6j Promote run-scoped cases, else they never run again
...
- **Every step is independently verified** — the load-bearing addition: each step runs
  ...
  STOP). Steps 3 and 6 are **hard STOPs** (don't dispatch Step 4 / don't file at 6f until APPROVE). The
  release gate (6h) is independently ratified. **Doer ≠ checker, always.**
...
- **Step 6j loop (pointer, not auto-trigger)** — FAIL/REOPEN → `/qa-fix` → human merge/deploy →
...
- **Promotion is execute → harvest → promote, and only the lifecycle promotes.** Cases are authored `Draft`
  (Step 3), executed as `Draft` (Step 4), and **6i harvests that execution as the `--verify` evidence** that
...
| `qa-lead` verifier | L1 | 1, 3, 4, 5, 6d/e, 6h | doer artifact + `{step, gate_criteria, source_of_truth, cmd?}` | `APPROVE`/`REJECT` + `REASONS`+`FIX` | delegates re-check to a **different** lane |
...
| `qa-backend-expert` | L3 | 4, 6a | same; triage oracle at 6a | same; App-Insights classification | edge |
| `qa-testing-expert` | L3 | 4/5 | exploratory charter | findings (bug/question/observation/risk) | firefox |
...
2. **Every atomic condition** (story ACs + gap-ACs) carries PASS/FAIL evidence and is reconciled live (6b).
3. **Every finding triaged** (class + provenance + severity + dedup, 6d); confirmed bugs filed with
   confirmation + `## Fix Routing` (6f).
4. A **verdict** (6e) issued, consistent with the triage table.
...
7. **Feature Release Gate fed** (6h) and independently ratified GO/CONDITIONAL/NO-GO.
8. **Promotion evidence harvested** (6i) when `new_cases_authored > 0` — assertions grounded to
...
9. **Close-out pointers stated** (6j), never auto-triggered: FAIL/REOPEN → `/qa-fix` → human merge/deploy
