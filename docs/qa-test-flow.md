...
- **FAST — a checklist, and nothing else.** A bug fix / copy-tweak / config / Technical task that is
  ...
  folder) → one execution agent → `5-triage`–`5-status`, then `5-docs`. It builds **no Test
  Model**, runs **no** archetype/UIP/`VC-*` sweeps, authors **no** test cases, and skips every independent
  verifier and `5-docs-map`. Two consequences are deliberate: a FAST run adds no regression
...
- **FULL — the whole pipeline.** A new feature / Epic, a **Story** (unless it is narrow on all six
  ...
  the **one** hard-STOP independent verifier (`3-cases`) plus 5-report's non-blocking ratification; the `1r`
...
step were **removed 2026-09-10** — they answered a *release* question at ~24 runner dispatches, and 5-verdict's
criteria are every one of them a claim about *this ticket*. Cutting a release means running
`/qa-regression … --cases critical` deliberately. Consequence, stated: 5-report's release gate ratifies its
...
sequenceDiagram
    ...
    note over Orch: 4a returned - CHECK the authoring agent. Complete? append, assemble C1 scope, gate. Still running? WAIT, never proceed to 5-triage
    Orch->>REG: 4c · C1 · /qa-regression <target suites> --ids <new Draft + REPAIR + RE-BASE ids> = every case this run wrote or changed
    REG-->>Orch: RUN_ID + pass rate (5-verdict.1 reconciles against it; the release gate does NOT)
    ...
    note over Orch,TRG: Step 5-triage · Triage — /qa-triage-results on the RUN_ID, then correlate + evidence-check
    ...
    note over Orch,V: Step 5-verdict.1 · Compare AC & DoD vs implementation (quantified estimate via qa-metrics)
    Orch->>Orch: reconcile ACs + DoD items live, compute AC-coverage%/DoD%
    Orch->>Orch: inline self-check, both paths (verify:gate --gate 5-verdict --run-id; the 5b verifier dispatch was folded away 2026-09-16)
    Orch->>Orch: 5-verdict.2 verdict (keyed off 5-triage provenance + 5-verdict.1 reconciliation)

    note over Orch,TR: Step 5-file · File bugs — sub-task (in-scope) / link-only (pre-existing) / standalone (incidental)
    opt Confirmed non-duplicate bugs
        Orch->>TR: 5-file file via /qa-bug (confirm, with Fix Routing + relationship)
    end

    note over Orch,TR: Step 5-report · Report — feed/ratify release gate, THEN post comment, THEN summary + chat
    ...
    note over Orch,TR: Step 5-status · Change status (strictly after the report is posted)
    Orch->>TR: TESTED (pass) / REOPEN (fail)
    note over Orch,TR: Step 5-docs · Publish documentation — after the transition, BOTH paths (refuses, never guesses)
    Orch->>TR: post the audience-routed guides as ONE comment (or state the refusal)
    opt FULL + a domain map EXISTS (5-docs-map · non-blocking)
...
### Diagram 2 — after the verdict (Step 5-report and the close-out branches)
...
sequenceDiagram
    ...
    alt PASS / PASS WITH NOTES
        Orch->>Gate: 5-report Feed verdict + regression pass rate + release criteria
    ...
    else FAIL  (--iterate: bounded loop, Step 5-loop, <= max-rounds)
...
- **Independent verification at the one hard-STOP gate (FULL path).** Step 3 (artifacts + data) runs
  `DOER → GATE → INDEPENDENT VERIFIER`. (The Feature Release Gate §1a inside Step 5-report is *also* independently
  ratified, but that's the verifier confirming a downstream recommendation off already-gated inputs, not a
  second from-scratch gate. Two more hard STOPs have left: `5g` when promotion left the pipeline on
  2026-09-10, and **Step 5b on 2026-09-16**, when its AC/DoD reconciliation became `5-verdict.1` and its REJECT
  criteria became `5-verdict`'s inline self-check —
  [`decisions/qa-test-evolution.md`](decisions/qa-test-evolution.md) §Folding 5b into 5-verdict.) The verifier is a **fresh `qa-lead-orchestrator` instance in
...
- **`1a` reads comments + analyzes attachments (both paths)** — the description is the plan, the comments
  ...
  site), `1d` (AC-affecting clarifications override a stale description), and Step 5-verdict.1 (the expected-vs-actual
...
- **Step 4 tracker gate** — Jira-only in-testing transition, deliberately unconfirmed (precondition for the
  Step 5-status close); the test-window start anchors the Step 5-triage App Insights correlation.
- **Step 5 order** — `5-triage` triage (incl. the `/qa-triage-results <RUN_ID> --fix` call on the Artifact-C run)
  runs **before** `5-verdict.1` AC/DoD reconciliation, which runs **before** `5-verdict.2` verdict, because PASS/FAIL is
  expressed in terms of a finding's provenance (from `5-triage`) and the reconciled AC/DoD state (from `5-verdict.1`).
  `5-triage` and `5-verdict` file nothing; `5-file` files; `5-status` transitions.
- **Step 5-triage triage** — the Artifact-C `RUN_ID`'s own FAILs are triaged via `/qa-triage-results <RUN_ID>
  ...
  *existing* suites, flakiness-history feed) instead of ad hoc re-derivation; findings without a RUN_ID
  ...
  tracker** — all before `5-file` files anything via `/qa-bug`. A test-defect routes to `/qa-review-tests
  --fix`, never a ticket. Only an in-scope P0/P1 (or an out-of-scope P0 revenue break) fails the verdict.
- **Step 5-verdict.1 AC/DoD reconciliation** — the gate the verdict rests on: reconciles every AC condition live,
  resolves any DoD items the `1e` Test Model deferred, and computes a **quantified estimate** (AC-coverage %,
  DoD-completion %) from the actual condition/checklist counts — `compute-metrics.ts` has no AC/DoD-shaped
  metric, so this is a simple deterministic ratio in `qa-metrics`' own style, not a repurposed
  regression-pass-rate number. **Self-checked inline on both paths** since 2026-09-16, against the criteria
  the removed FULL-path verifier used to reject on.
- **Step 5-file filing — relationship by provenance.** **IN-SCOPE → filed as a tracker Sub-task of the ticket**
...
- **Step 5-report Report, before Step 5-status status change.** The tracker comment (QA Complete summary, incl. the
...
- **`--iterate` — the bounded test → fix → re-test loop (opt-in, Step 5-loop)** — with `--iterate` (default
  ...
  **`5-loop.0` round entry: probes the deployed build, then re-reads the board** — the ticket’s sub-tasks and
  ...
  re-verdicts. **No suite sweep runs inside the loop** — the round's own decision comes from 5-verdict, which never
  depended on one. PASS exits to the Feature Release Gate (5-report); still-FAIL at the cap STOPs with a
  ...
  **It re-persists as well as re-runs, and the split is the contract:** per round it files new bugs
  ...
  checklist; the release gate, the full QA-Complete comment, the tracker transition and `5-docs`/`5-docs-map` all
  ...
  [`skills/qa-test/modes.md`](../.claude/skills/qa-test/modes.md) §5-loop.
...
| The story run | Step 5-triage triage + Step 5-verdict.1 AC/DoD vs implementation (incl. the quantified estimate) + Step 5-verdict.2 verdict — every AC condition carries PASS evidence, all reconciled SATISFIED-live, DoD items MET/N-A, all `BL-*` verified, no **in-scope** P0/P1 bug, no correlated App-Insights REAL_BUG | PASS / PASS WITH NOTES / FAIL / BLOCKED | [`commands/qa-test.md`](../.claude/commands/qa-test.md) §5-triage–5-verdict |
...
flowchart TB
    ...
    subgraph L1["L1 · Verification (fresh instance, FULL path, Step 3, hard STOP; Step 5-report, non-blocking)"]
    ...
    H -->|VCST-XXXX| O
    O -->|gate ruling req (Step 3, full path)| V
...
| `qa-lead` verifier | L1 | 3 (hard STOP) + 5-report (non-blocking), full path | doer artifact + `{step, gate_criteria, source_of_truth, cmd?}` | `APPROVE`/`REJECT` + `REASONS`+`FIX` | delegates re-check to a **different** lane |
...
| `qa-backend-expert` | L3 | 4, 5-triage | same; triage oracle at 5-triage | same; App-Insights classification | edge |
...
| `/qa-triage-results` | L3 | 5-triage | `RUN_ID` + `--fix` | `triage-report.md` (confirmed bugs / test-fixes applied / dismissed) + flakiness history feed | own pool |
...
| **4** Execution | every condition has PASS/FAIL evidence **and says which pass produced it (4a or 4b)**; regression `RUN_ID`+rate exist (the run also executed the new Draft cases) | inline self-check | inline |
| **5-triage** Triage | Artifact-C `RUN_ID`'s FAILs triaged via `/qa-triage-results --fix` (not ad hoc); every other finding classified + provenance + severity + deduped | folded into the Step 5-verdict gate below | inline (no separate dispatch) |
| **5-verdict.1** AC & DoD vs implementation | every AC condition reconciled; every DoD item resolved MET/NOT-MET/N-A; AC-coverage % + DoD % computed from actual counts | inline self-check, both paths — re-derive from the Step-4 evidence, `verify:gate --gate 5-verdict --run-id <ID>` for the deterministic half | **gates 5-verdict.2** |
| **5-file** File bugs | IN-SCOPE → Sub-task of the ticket; PRE-EXISTING → linked, not re-filed; incidental → standalone + related link | inline self-check (mechanical — 5-triage graded the provenance/severity calls and 5-verdict settled them) | inline |
| **5-report** Report → release gate | §1a criteria → GO/CONDITIONAL/NO-GO | **FULL:** V re-evaluates `compute-metrics.ts --gate feature --run-id <RUN_ID>` (scope required; exit 2 = CANNOT EVALUATE, not a failure) + open-bug ledger (now current, post-5-file) | ratify/downgrade |
| **5-docs** Publish documentation | audiences derived from `layer`; size caps honoured; a refusal (`layer-unresolved` / `not-deployed` / `not-user-visible`) is STATED, never a silent skip | inline self-check | inline |
| **5-docs-map** Domain-map write-back *(FULL, only when a map exists)* | every appended claim is live-`CONFIRMED` with the ticket id; no row deleted, no id renumbered; `amended` moves and `generated`/`rev` do not | inline self-check + `domain:check` + `context:check` | inline, non-blocking |

### End of flow — DoD

A `/qa-test` run is **Done** when all hold:

1. **The FULL-path hard-STOP gate APPROVED** (`3-cases`) — or the FAST path's inline
...
2. **Every atomic condition** (story ACs + gap-ACs) carries PASS/FAIL evidence and is reconciled live, and
   every DoD item is resolved MET/NOT-MET/N-A, with the AC-coverage/DoD percentages computed (5-verdict.1).
3. **Every finding triaged** (class + provenance + severity + dedup, 5-triage); confirmed bugs filed with
   confirmation, the right tracker relationship (Sub-task / link / standalone), and `## Fix Routing` (5-file).
4. A **verdict** (5-verdict) issued, consistent with the triage + AC/DoD tables.
...
6. **QA comment posted, then tracker transitioned** to TESTED/REOPEN — the terminal reach; never
   Done/Cancelled (5-report then 5-status, in that order).
7. **Feature Release Gate fed** (5-report) and independently ratified GO/CONDITIONAL/NO-GO.
8. **Documentation published (5-docs)** — or its refusal stated — and, on FULL with an existing domain map,
   **`5-docs-map` has written back what the run verified** (or recorded `NOTHING_TO_AMEND`). New cases remain
