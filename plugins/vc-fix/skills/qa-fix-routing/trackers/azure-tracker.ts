...
// Azure HTML conversion + the Bug JSON-Patch builder are shared with the CLI (ado.mjs) via one
// module, so the two can't drift. tsx resolves the .mjs at runtime; ado-html.d.mts carries types.
import { ensureAzureHtml, buildBugFields } from "../ado-html.mjs";
...
export class AzureTracker implements Tracker {
  ...
  // builder ado.mjs's create-workitem uses — so the field list can't drift between the two.
  async createWorkItem(
    ...
    // The Bug JSON-Patch body is built by the SHARED buildBugFields (same source as ado.mjs's
    // create-workitem — no drift). The CLI resolves --assign-self / --iteration current to
    // concrete values first; here the caller passes already-resolved email / path / id.
    ...
    // The leading `$` before the type is required by the create endpoint.
    const res = await this.req(
      "POST",
