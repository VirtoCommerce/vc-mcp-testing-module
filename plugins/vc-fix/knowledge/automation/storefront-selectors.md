...
**Purpose:** Source of truth for selectors used by layout/shift audits (`critical-ui-scope.md` protocols) and other storefront UI tests. Previously also backed suite `048b-layout-stability.csv`, removed 2026-07-25. Every selector below was verified live with `document.querySelectorAll(...).length` returning the documented `matched` count.
