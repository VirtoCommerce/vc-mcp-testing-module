...
- **Source (read-side anchor, added 2026-09-08):** `VirtoCommerce/vc-module-loyalty` @ `dev` — `src/VirtoCommerce.Loyalty.ExperienceApi/Extensions/DataLoaderContextAccessorExtensions.cs` → `LoadLoyaltyObject()`: `objectType switch { nameof(CustomerOrder) => loader.LoadAsync(objectId), nameof(ApplicationUser) => … Type = "Registration", _ => …Task.FromResult<LoyaltyOperationLogObject>(null) }`. `"LoyaltyMissionProgress"` falls to the default arm, so the `object` field defined in `Schemas/LoyaltyOperationLogType.cs` resolves to null. This is the exact line the invariant now turns on. Deployed artifact when read: `VirtoCommerce.Loyalty 3.1006.0` / `VirtoCommerce.Xapi 3.1020.0`; live behaviour matches this source.
- **Measured on the xAPI surface — a mission grant projects NO attribution.** Live read of the VIP fixture's `Earned` ledger (2026-08-28, 63 entries): **17 entries have `object === null`** — no order reference of any kind — and the four known mission rewards (`750`, `450`, `550`, `400`) are all in that group, one each, none carrying an object. Every one of the other 46 entries, which are product-earn, carries `type: CustomerOrder` plus an `orderNumber`. So through GraphQL the ledger distinguishes the two earn paths precisely by whether it records where the points came from. Re-read 2026-09-08 on the same fixture, now **69 of 167** entries: unchanged shape.
