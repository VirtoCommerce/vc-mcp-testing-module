...
- **Note:** canonical capture pattern is via follow-up mutation response, NOT addItem response itself
...
- **Note:** use `subTotal` arithmetic for multi-add assertions, NOT `itemsCount`.
...
- **Note:** the per-processor payment flows are also described in `agents/qa-testing-expert.md` §Payment.
...
- **Pattern:** Skyflow's test vault accepts only the canonical card in `SKYFLOW_VISA` / `SKYFLOW_MASTERCARD`; any other test card will be rejected even if it's a valid Visa/MC test number
