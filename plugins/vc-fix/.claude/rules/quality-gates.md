...
| **G6** | E2E / deploy verification | `qa-backend-expert` (module/platform) / `qa-frontend-expert` (vc-frontend) | **Backend is static-only in CI** — the live symptom needs a redeploy; once the PR's alpha artifact deploys to QA, `qa-backend-expert` reproduces the original STR live against the deployed artifact (Backend suites from `module-suite-map.md` are a reference for what's covered, but `/qa-regression` itself is full `vc-qa` plugin only, not shipped here) = pass (rerun once on a single flaky). **Frontend** logic is unit-proven, but visual / CLS aspects need a deploy — `qa-frontend-expert` re-verifies the fix live on the deployed artifact. **Admin SPA layout/CSS** was already proven pre-PR by the render harness (G3); G6 is the final real-user confirmation once the module's alpha artifact deploys to QA. If no deploy yet, the PR is **labeled "needs deploy/visual verification"** and G6 closes post-merge via `/qa-verify-fix` | Persistent RED or cross-repo need → STOP + hand off. |
