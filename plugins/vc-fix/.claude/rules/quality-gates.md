...
| **G6** | E2E / deploy verification | `qa-backend-expert` (module/platform) / `qa-frontend-expert` (vc-frontend) | **Self-sufficient without `/qa-regression`** (full `vc-qa` plugin only, not shipped here) — G6 is a direct, live re-run of the bug's own STR by the QA expert against the deployed artifact, not a suite execution; nothing in this gate depends on the regression pipeline. **Backend is static-only in CI** — the live symptom needs a redeploy; once the PR's alpha artifact deploys to QA, `qa-backend-expert` reproduces the original STR live against the deployed artifact (`module-suite-map.md` is consulted only as a reference for which module/area owns the fix, not a runnable suite selector) = pass (rerun once on a single flaky). **Frontend** logic is unit-proven, but visual / CLS aspects need a deploy — `qa-frontend-expert` re-verifies the fix live on the deployed artifact the same way. **Admin SPA layout/CSS** was already proven pre-PR by the render harness (G3); G6 is the final real-user confirmation once the module's alpha artifact deploys to QA. If no deploy yet, the PR is **labeled "needs deploy/visual verification"** and G6 closes post-merge via `/qa-verify-fix` | Persistent RED or cross-repo need → STOP + hand off. |
