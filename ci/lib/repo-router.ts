 ...
 * `src/VirtoCommerce.PageBuilderModule.Web/Apps/page-builder-shell/`). Declared per-repo in `ci/config/fix-repos.json`
 * `moduleFrontendSubApps`; resolved by RCA anchor via `resolveOwningSubApp()`. `stack` is a
 * closed enum — supporting a new stack is a new agent capability, not just a config row.
 */
export interface SubAppProfile {
  /** Repo-relative directory of the sub-app, no leading `./` (e.g. "src/VirtoCommerce.PageBuilderModule.Web/Apps/page-builder-shell"). */
  path: string;
  stack: "vue3";
...
export interface SubAppOverride {
  ...
  /** Toolchain profile to use for THIS bug in place of `REPO_PROFILES[kind]`. `kind` is
   *  unchanged (still "module") — ownership/allowlist/PR-target logic is untouched. */
  profile: RepoProfile;
}
 ...
 * `moduleFrontendSubApps` has zero effect on `isAllowedRepo`, `repoOwnership`, or
 * `contributionPlan` for this repo, and zero effect on any repo with no declared entry.
 */
