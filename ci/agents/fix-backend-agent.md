...
- **Logic** (save/payload, computed value, wrong endpoint, binding condition) → prove red→green with an uncommitted **Node scratch harness** (`.fix-workspace/_scratch/VCST-XXXX/repro.cjs`; modules have NO JS test harness — never scaffold one). See `.claude/skills/development/angular-admin/scratch-harness-patterns.md`.
