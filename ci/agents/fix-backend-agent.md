...
- **Not your lane: an embedded frontend sub-app on a different stack.** Some hybrid module repos ship one or more standalone frontend apps under an `Apps/<name>/` directory (typically nested under the `.Web` project, e.g. `src/VirtoCommerce.PageBuilderModule.Web/Apps/`) on a stack other than this module's own C#/legacy-AngularJS `Web/Scripts/` (e.g. `vc-module-pagebuilder`'s Vue 3 "shell" at `src/VirtoCommerce.PageBuilderModule.Web/Apps/page-builder-shell/`, or its separate Angular 21 "designer"). If the RCA anchor lies outside `Web/Scripts/` and outside the module's `.Core`/`.Data`/`.Web` C# projects — i.e. under an `Apps/<name>/` path — **STOP immediately**: `FIX_STATUS: FAILED`, `ROOT_CAUSE: embedded frontend sub-app <path>, not a CI-supported stack yet`. Do not attempt a C#/AngularJS-shaped fix against a different framework. (The interactive `/qa-fix` twin has a routing mechanism for the Vue 3 case — `resolveOwningSubApp()` in `ci/lib/repo-router.ts` + the `/vc-shell-fix` skill — that this headless CI agent does not yet call; that wiring is a tracked follow-up, not something to approximate here.)
