...
1. **Understand the bug.** Read the ticket JSON and bug report. Extract the STR, expected vs actual, and any root-cause hint. Locate the responsible component/composable/store (`Grep`/`Glob` for selectors, text, `data-test-id`s, route names, i18n keys, GraphQL operations — or, once install has run (step 2), an LSP-backed code-navigation tool if this CI environment has one available, e.g. Serena's `get_symbols_overview`/`find_symbol`, which resolves the seam directly instead of a blind Grep). Two storefront traps to rule out before "fixing": (a) a symptom that only appears when a **`$cfg.*` feature flag** is on/off is **configuration, not a code bug** → `FIX_STATUS: FAILED` (config-gated, not code); (b) a wrong **GraphQL field name** no-ops silently to `undefined` — verify the real field against the live schema / generated types before changing a mapping.
