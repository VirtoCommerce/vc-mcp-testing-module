...
| File | Role |
|------|------|
| `skills/project-init/SKILL.md` | methodology (steps 0–8) |
| `commands/project-init.md` | command (compact mirror) |
| `skills/project-init/scaffold-env.mjs` | `.env.<env>` template |
| `skills/project-init/scaffold-secrets.mjs` | `.env.local` template (auth axes `--ado-auth`/`--github-auth`/`--jira-auth`) |
| `skills/project-init/gen-profile.mjs` | writes `project-profile.json` from flags |
| `skills/project-init/discover-repos.mjs` | scan modules + client repos (classify + theme scan) |
| `skills/project-init/gen-mcp.mjs` | `.mcp.json` |
| `skills/project-init/verify-access.mjs` | readiness table + real probes (ADO/GitHub/admin) |
| `skills/project-init/ensure-session.mjs` | browser login without hand-typed commands (auto-tenant + az/gh login) |
...
Local verify: `FORCE_COLOR=1 TEST_ENV=<env> node skills/project-init/verify-access.mjs`.
