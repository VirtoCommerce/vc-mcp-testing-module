...
- **Check:** Compare against known admin route patterns from `knowledge/domain/sitemap.md`.

### DV-006: Invalid GraphQL query/mutation name `[Blocker]`
- **Detection:** (GraphQL suites only) Steps contain a query or mutation name that does not exist in the live schema. Reference: `knowledge/api/graphql-schema.md`.
