...
**Probe:** Pick a module (e.g., `vc-module-marketing`). Open the [module-suite-map](../../knowledge/execution/module-suite-map.md) to find its suite(s). Open the suite CSV(s). Then scan the module's GitHub source via `mcp__github__search_code` for handlers, endpoints, or commands that don't appear in the suite.
...
Discovery sessions hit a moving target: catalogs drift, orgs get reseeded, cart IDs change. Hardcoding values during exploration causes false negatives (404 on a known ID) that mask the real discovery. Use the decision tree in [live-discovery.md](../../knowledge/execution/live-discovery.md):
...
Cross-ref: [../../../rules/test-data.md](../../.claude/rules/test-data.md) is the project-wide policy.
